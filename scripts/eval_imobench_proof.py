    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert IMO grader. Provide rigorous assessment.",
                },
        ...
        has_critical_errors = (
            errors_match and "None" not in errors_match.group(1)
            if errors_match
            else False
        )
...
def analyze_results(results: List[Dict]):
    ...
    partial_credit = sum(
        1 for r in results if 1 <= r["verification"].get("score", 0) <= 6
    )
    ...
    basic_full = sum(
        1
        for r in results
        if "Basic" in r.get("problem_id", "") and r["verification"].get("score", 0) == 7
    )
    basic_total = sum(1 for r in results if "Basic" in r.get("problem_id", ""))
    adv_full = sum(
        1
        for r in results
        if "Advanced" in r.get("problem_id", "")
        and r["verification"].get("score", 0) == 7
    )
    ...
    print(
        f"Advanced problems: {adv_full}/{adv_total} = {adv_full/adv_total*100 if adv_total > 0 else 0:.1f}%"
    )
    ...
    print(
        f"Partial credit (1-6): {partial_credit} ({partial_credit/total_problems*100:.1f}%)"
    )
    print(f"No credit (0): {no_credit} ({no_credit/total_problems*100:.1f}%)")

    # Basic vs Advanced (average scores)
    basic_scores = [
        r["verification"].get("score", 0)
        for r in results
        if "Basic" in r.get("problem_id", "")
    ]
    adv_scores = [
        r["verification"].get("score", 0)
        for r in results
    ...
    if basic_scores or adv_scores:
        ...
        if basic_scores:
            ...
            print(
                f"Basic ({len(basic_scores)}):    {basic_avg:.2f}/7 ({basic_avg/7*100:.1f}%)"
            )
        if adv_scores:
            adv_avg = sum(adv_scores) / len(adv_scores)
            print(
                f"Advanced ({len(adv_scores)}): {adv_avg:.2f}/7 ({adv_avg/7*100:.1f}%)"
            )
    ...
    if categories:
        ...
        for cat, stats in sorted(categories.items()):
            ...
            total = stats["total"]
            print(
                f"{cat:20s}: Avg {avg:.2f}/7 ({avg/7*100:.1f}%) | Solved {full}/{total} ({full/total*100:.1f}%)"
            )
    ...
    if levels:
        ...
        for level, stats in sorted(levels.items()):
            ...
            total = stats["total"]
            print(
                f"{level:20s}: Avg {avg:.2f}/7 ({avg/7*100:.1f}%) | Solved {full}/{total} ({full/total*100:.1f}%)"
            )
    ...
    try:
        ...
        reasoning_tokens = sum(
            r["response"].get("reasoning_tokens", 0) for r in results
        )
    ...
    print("\n" + "-" * 80)
    print(
        f"Total solve time: {total_time:.1f}s ({total_time/60:.1f} minutes, {total_time/3600:.1f} hours)"
    )
...
def main():
    ...
    parser.add_argument(
        "--timeout", type=int, default=600, help="Timeout in seconds for each problem"
    )
    parser.add_argument(
        "--limit", type=int, default=None, help="Limit number of problems (for testing)"
    )
    parser.add_argument(
        "--subset",
        type=str,
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Solving proof problems"):
        ...
        response = get_llm_response(
            problem_text, args.model, client, extra_body=None, timeout=args.timeout
        )
