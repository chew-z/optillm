...
class OptILMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "approaches": torch.tensor(
                [APPROACHES.index(approach) for approach in approaches],
                dtype=torch.long,
            ),
...
def load_and_preprocess_data(tokenizer):
    ...
    for item in dataset["train"]:
        ...
        valid_results = [
            ...
            if result["rank"] is not None and "tokens" in result
        ]
    ...
    approach_counts = Counter(
        approach for item in data_items for approach in item["approaches"]
    )
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            logits = model(
                input_ids, attention_mask, effort[:, 0]
            )  # Use effort for the best approach
            ...
            effort_loss = F.mse_loss(
                ...
                .gather(1, best_approach_indices.unsqueeze(1))
                .squeeze(),
                effort[:, 0],
            )
            ...
            # Combine losses
            loss = (
                ce_loss + 0.1 * effort_loss
            )  # Adjust the weight of effort_loss as needed
        ...
        print(
            f"Epoch {epoch+1}/{num_epochs}, Train Loss: {avg_train_loss:.4f}, Train Accuracy: {avg_train_accuracy:.4f}, Val Accuracy: {avg_val_accuracy:.4f}"
        )
...
def inference(model, tokenizer, prompt, effort_levels):
    ...
    with torch.no_grad():
        ...
        for effort in effort_levels:
            ...
            results.append(
                (
                    APPROACHES[predicted_approach_index],
                    probabilities[0][predicted_approach_index].item(),
                )
            )
...
def main(args):
    ...
    for prompt in test_prompts:
        ...
        for effort, (approach, confidence) in zip(effort_levels, results):
            print(
                f"Effort: {effort:.1f}, Predicted Approach: {approach}, Confidence: {confidence:.4f}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--hub_model_id", type=str, help="Model ID for Hugging Face Hub"
    )
    parser.add_argument(
