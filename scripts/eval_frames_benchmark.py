...
client = OpenAI(
    api_key=os.environ.get("OPENAI_API_KEY"), base_url="http://localhost:8000/v1"
)
# client = OpenAI()
SLEEP_INTERVAL = 300
...
def main(model: str):
    ...
    for item in tqdm(dataset, desc="Processing samples"):
        ...
        evaluation = evaluate_response(
            item["Prompt"], llm_response, item["Answer"], model
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate LLM performance on google/frames-benchmark"
    )
    parser.add_argument(
        "--model",
        type=str,
