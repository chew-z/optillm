...
THOUGHT_TRANSITIONS = ["Wait,", "Alternatively,", "However,", "Additionally,"]
...
def load_2024_dataset() -> list[dict]:
    ...
    assert (
        len(dataset) == 30
    ), f"Expected 30 problems after filtering by 2024, but found {len(dataset)}"
    return dataset
...
def load_2025_dataset() -> list[dict]:
    ...
    assert (
        len(dataset) == 30
    ), f"Expected 30 problems in AIME 2025, but found {len(dataset)}"
    return dataset
...
def extract_answer(response: str) -> Optional[int]:
    ...
    extracted_answer = unified_extract_answer(
        response, problem_type="aime", problem_id=None
    )
...
def get_llm_response(
    ...
    try:
        ...
        response = client.with_options(timeout=6000.0).chat.completions.create(
            model=model,
            messages=[{"role": "user", "content": SYSTEM_PROMPT + problem}],
            max_tokens=64000,
            **kwargs,
        )
        ...
        if len(response.choices) > 1:
            ...
            for i, choice in enumerate(response.choices):
                ...
                if (
                    analyze_logits
                    and hasattr(choice.message, "logprobs")
                    and choice.message.logprobs
                ):
        ...
        if (
            analyze_logits
            and hasattr(response.choices[0].message, "logprobs")
            and response.choices[0].message.logprobs
        ):
            return {
                "response": response_text,
        ...
        if "timeout" in str(e).lower():
            logger.error(
                "API call timed out - consider increasing timeout for complex approaches like MARS"
            )
...
def make_n_attempts(
    ...
    model: str,
    n: int,
    analyze_thoughts: bool = False,
    analyze_logits: bool = False,
    ...
    while remaining_attempts > 0:
        try:
            ...
            logger.error(
                f"Failed to get response for attempt {n - remaining_attempts + 1}: {e}"
            )
        ...
        if isinstance(response, list):
            for attempt in response:
                ...
                if analyze_logits and "logprobs" in attempt:
                    attempt["logit_analysis"] = analyze_logits_probs(
                        attempt["logprobs"]["content"]
                    )
            ...
            if analyze_logits and "logprobs" in response:
                attempt_data["logit_analysis"] = analyze_logits_probs(
...
def analyze_results(
    ...
    if analyze_thoughts:
        ...
        for result in results:
            for attempt in result["attempts"]:
                if "thought_analysis" in attempt:
                    if (
                        result["is_correct"]
                        and attempt["predicted_answer"] == result["correct_answer"]
                    ):
        ...
        def calc_stats(attempts):
            ...
            thinking_tokens = [
                a["thought_analysis"]["thinking_tokens"] for a in attempts
            ]
            thought_transitions = [
                a["thought_analysis"]["thought_transitions"] for a in attempts
            ]
            has_think_tags = sum(
                1 for a in attempts if a["thought_analysis"]["has_think_tags"]
            )

            # Count total transition usage
            transition_usage = defaultdict(int)
            for attempt in attempts:
                for phrase, count in attempt["thought_analysis"][
                    "transition_counts"
                ].items():
            ...
            return {
                "count": len(attempts),
                "avg_thinking_tokens": (
                    statistics.mean(thinking_tokens) if thinking_tokens else 0
                ),
                "median_thinking_tokens": (
                    statistics.median(thinking_tokens) if thinking_tokens else 0
                ),
                ...
                "avg_thought_transitions": (
                    statistics.mean(thought_transitions) if thought_transitions else 0
                ),
                "median_thought_transitions": (
                    statistics.median(thought_transitions) if thought_transitions else 0
                ),
                "transition_usage": dict(transition_usage),
                "has_think_tags_pct": (
                    (has_think_tags / len(attempts)) * 100 if attempts else 0
                ),
            }
        ...
        print(
            f"- Range: {all_stats['min_thinking_tokens']} - {all_stats['max_thinking_tokens']} tokens"
        )
        print(
            f"- Average thought transitions: {all_stats['avg_thought_transitions']:.2f}"
        )
        print(
            f"- Median thought transitions: {all_stats['median_thought_transitions']}"
        )
        ...
        print(
            f"- Average thought transitions: {correct_stats['avg_thought_transitions']:.2f}"
        )
        print(
            f"- Median thought transitions: {correct_stats['median_thought_transitions']}"
        )
        print(
            f"- Percentage with <think> tags: {correct_stats['has_think_tags_pct']:.2f}%"
        )
        print("- Transition phrase usage:")
        ...
        print(
            f"- Average thinking tokens: {incorrect_stats['avg_thinking_tokens']:.2f}"
        )
        print(f"- Median thinking tokens: {incorrect_stats['median_thinking_tokens']}")
        print(
            f"- Average thought transitions: {incorrect_stats['avg_thought_transitions']:.2f}"
        )
        print(
            f"- Median thought transitions: {incorrect_stats['median_thought_transitions']}"
        )
        print(
            f"- Percentage with <think> tags: {incorrect_stats['has_think_tags_pct']:.2f}%"
        )
        print("- Transition phrase usage:")
        ...
        if correct_attempts and incorrect_attempts:
            ...
            for result in results:
                ...
                for attempt in result["attempts"]:
                    if "thought_analysis" in attempt:
                        category = (
                            "correct"
                            if attempt["predicted_answer"] == result["correct_answer"]
                            else "incorrect"
                        )
            ...
            valid_problems = {
                k: v
                for k, v in problems_with_both.items()
                if v["correct"] and v["incorrect"]
            }

            if valid_problems:
                print(
                    f"Found {len(valid_problems)} problems with both correct and incorrect attempts"
                )
                ...
                for problem_id, attempts in valid_problems.items():
                    correct_tokens = [
                        a["thought_analysis"]["thinking_tokens"]
                        for a in attempts["correct"]
                    ]
                    incorrect_tokens = [
                        a["thought_analysis"]["thinking_tokens"]
                        for a in attempts["incorrect"]
                    ]

                    correct_transitions = [
                        a["thought_analysis"]["thought_transitions"]
                        for a in attempts["correct"]
                    ]
                    ...
                    avg_correct_tokens = (
                        statistics.mean(correct_tokens) if correct_tokens else 0
                    )
                    avg_incorrect_tokens = (
                        statistics.mean(incorrect_tokens) if incorrect_tokens else 0
                    )

                    avg_correct_transitions = (
                        statistics.mean(correct_transitions)
                        if correct_transitions
                        else 0
                    )
                    avg_incorrect_transitions = (
                        statistics.mean(incorrect_transitions)
                        if incorrect_transitions
                        else 0
                    )

                    avg_token_diff.append(avg_correct_tokens - avg_incorrect_tokens)
                    avg_transition_diff.append(
                        avg_correct_transitions - avg_incorrect_transitions
                    )

                print(
                    f"Average token difference (correct - incorrect): {statistics.mean(avg_token_diff):.2f}"
                )
                print(
                    f"Average transition difference (correct - incorrect): {statistics.mean(avg_transition_diff):.2f}"
                )
    ...
    if analyze_logits:
        ...
        def calc_logit_stats(attempts):
            ...
            for attempt in attempts:
                if attempt["logit_analysis"].get("entropy_stats") and attempt[
                    "logit_analysis"
                ]["entropy_stats"].get("mean"):
                    entropy_means.append(
                        attempt["logit_analysis"]["entropy_stats"]["mean"]
                    )
                    entropy_stds.append(
                        attempt["logit_analysis"]["entropy_stats"]["std"]
                    )

                    if attempt["logit_analysis"]["entropy_stats"].get("quartiles"):
                        entropy_quartiles.append(
                            attempt["logit_analysis"]["entropy_stats"]["quartiles"]
                        )

                    # Collect transition entropy data
                    if attempt["logit_analysis"].get("transition_entropy"):
                        for phrase, stats in attempt["logit_analysis"][
                            "transition_entropy"
                        ].items():
                            if stats.get("before_mean") is not None:
                                transition_entropies[phrase]["before"].append(
                                    stats["before_mean"]
                                )
                            if stats.get("after_mean") is not None:
                                transition_entropies[phrase]["after"].append(
                                    stats["after_mean"]
                                )
            ...
            if entropy_quartiles:
                ...
                padded_quartiles = [
                    q + [0] * (max_quartiles - len(q)) for q in entropy_quartiles
                ]
                ...
                for i in range(max_quartiles):
                    ...
                    avg_quartiles.append(
                        statistics.mean(quartile_values) if quartile_values else 0
                    )
        ...
        if all_stats["entropy"] and all_stats["entropy"]["mean"]:
            ...
            if all_stats["transitions"]:
                ...
                for phrase, stats in all_stats["transitions"].items():
                    ...
                    print(
                        f"  - {phrase} (n={stats['count']}): Entropy {change_dir} by {abs(change):.4f}"
                    )
                    print(
                        f"    - Before: {stats['before_mean']:.4f}, After: {stats['after_mean']:.4f}"
                    )
        ...
        if correct_stats["count"] > 0 and incorrect_stats["count"] > 0:
            ...
            if (
                correct_stats["entropy"]
                and correct_stats["entropy"]["mean"]
                and incorrect_stats["entropy"]
                and incorrect_stats["entropy"]["mean"]
            ):
                ...
                if (
                    correct_stats["entropy"]["quartiles"]
                    and incorrect_stats["entropy"]["quartiles"]
                ):
                    print("- Entropy progression through generation:")

                    for i in range(
                        min(
                            len(correct_stats["entropy"]["quartiles"]),
                        ...
                        print(
                            f"  - Q{i+1}: Correct: {c_q:.4f}, Incorrect: {i_q:.4f}, Diff: {q_diff:.4f}"
                        )

                # Compare transitions
                common_transitions = set(correct_stats["transitions"].keys()) & set(
                    incorrect_stats["transitions"].keys()
                )
                ...
                if common_transitions:
                    ...
                    for phrase in common_transitions:
                        ...
                        print(f"  - {phrase}:")
                        print(
                            f"    - Correct: {c_stats['before_mean']:.4f} → {c_stats['after_mean']:.4f} (Δ {c_change:.4f})"
                        )
                        print(
                            f"    - Incorrect: {i_stats['before_mean']:.4f} → {i_stats['after_mean']:.4f} (Δ {i_change:.4f})"
                        )
                        print(
                            f"    - Difference in entropy change: {c_change - i_change:.4f}"
                        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate LLM performance on AIME problems"
    )
    parser.add_argument(
        "--model",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of attempts per problem (for pass@n evaluation)",
    )
    parser.add_argument(
        "--year",
        type=int,
        default=2024,
        choices=[2024, 2025],
        help="AIME year to evaluate (2024 or 2025)",
    )
    parser.add_argument(
        "--approach", type=str, help="OptILLM approach to use (e.g., mars, moa, bon)"
    )
    parser.add_argument(
        "--analyze-thoughts",
        action="store_true",
        help="Analyze thinking patterns in responses",
    )
    parser.add_argument(
        "--analyze-logits",
        action="store_true",
        help="Analyze token probability distributions",
    )
    parser.add_argument(
        "--test-time-compute",
        action="store_true",
        help="Evaluate test-time compute scaling approaches",
    )
    ...
    if args.test_time_compute:
        ...
        TEST_TIME_COMPUTE_APPROACHES = [
            ...
            (
                "thinkdeeper_2k",
                "ThinkDeeper with 2K thinking tokens",
                {
            ...
            (
                "thinkdeeper_4k",
                "ThinkDeeper with 4K thinking tokens",
                {
                    "decoding": "thinkdeeper",
        ...
        for approach_slug, approach_name, extra_body in TEST_TIME_COMPUTE_APPROACHES:
            ...
            main(
                ...
                args.n,
                args.year,
                args.analyze_thoughts,
                args.analyze_logits,
                test_time_compute=True,
                approach_name=approach_slug,
        ...
        extra_body = (
            ...
            if args.approach and args.approach != "none"
            else None
        )
        approach_name = (
            args.approach if args.approach and args.approach != "none" else None
        )
