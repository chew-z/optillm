        ...
        if question:
            ...
            if correct_index >= 0:
                ...
                if is_numeric and value == correct_index:
                    logger.debug(
                        f"Numeric match: response '{response}' -> {value} matches index {correct_index}"
                    )
                    return True

                # Case 3: Check if response is "index. answer"
                if re.search(
                    rf"{correct_index}\s*\.\s*{re.escape(ground_truth_clean)}",
                    response_clean,
                ):
                ...
                if (
                    str(correct_index) in response_clean
                    and ground_truth_clean in response_clean
                ):
...
def evaluate_model(
    ...
    for example in tqdm(examples, desc=f"Evaluating {approach}"):
        ...
        if n_param > 1 and (
            approach.startswith("avg@") or approach.startswith("pass@")
        ):
            try:
                # Get appropriate prompt for the category
                prompt = get_prompt_for_category(
                    example["question"], example["category"]
                )
                ...
                if approach_extra_body:
                    extra_body_clean = {
                        k: v
                        for k, v in approach_extra_body.items()
                        if k not in ["n", "approach"]
                    }
                ...
                try:
                    ...
                    responses = [
                        (choice.message.content, time.time() - start_time)
                        for choice in response.choices
                    ]
                    logger.debug(
                        f"Generated {len(responses)} responses using n={n_param}"
                    )

                except Exception as e:
                    # If n parameter fails, fall back to sequential generation
                    logger.warning(
                        f"Parallel generation failed: {type(e).__name__}: {str(e)}"
                    )
                    ...
                    for i in range(n_param):
                        try:
                            ...
                            responses.append(
                                (response_text, time.time() - single_start)
                            )
                        except Exception as seq_error:
                            logger.error(
                                f"Sequential generation {i+1}/{n_param} failed: {seq_error}"
                            )
                ...
                if approach.startswith("avg@"):
                    ...
                    success_rate = (
                        sum(run_results) / len(run_results) if run_results else 0
                    )
        ...
        # For pass@1, we need to run multiple times and calculate average
            ...
            for run_idx in range(num_runs):
                try:
                    ...
                    logger.error(
                        f"Error in run {run_idx+1} for example {example['id']}: {e}"
                    )
            ...
            try:
                ...
                if approach_extra_body:
                    extra_body_clean = {
                        k: v for k, v in approach_extra_body.items() if k != "approach"
                    }
                    extra_body.update(extra_body_clean)
                ...
                detailed_results.append(
                    {
                        ...
                        "processed_response": (
                            processed_response if has_thinking else None
                        ),
    ...
    final_metrics = {
        "accuracy": (
            metrics["total_correct"] / metrics["samples"]
            if metrics["samples"] > 0
            else 0
        ),
        "average_time": (
            metrics["total_time"] / metrics["samples"] if metrics["samples"] > 0 else 0
        ),
    ...
    if failures > 0:
        logger.warning(
            f"Approach {approach}: {failures}/{total_expected} examples failed due to errors"
        )
        logger.warning(
            "Failed examples are counted as incorrect in accuracy calculation"
        )

    # Add category-specific metrics
    for category, cat_metrics in category_metrics.items():
        final_metrics[f"{category}_accuracy"] = (
            cat_metrics["correct"] / cat_metrics["total"]
        )
        final_metrics[f"{category}_average_time"] = (
            cat_metrics["time"] / cat_metrics["total"]
        )
        ...
        [
            {
                ...
                if k != "raw_response" and k != "processed_response"
            }
...
def generate_report(
    ...
    if is_default_test_time:
        ...
        report.append(
            "All approaches use n=5 parallel generation (with sequential fallback) for fair comparison.\n"
        )
    elif is_test_time_compute:
        report.append("This report evaluates test-time compute scaling approaches:")
        report.append(
            "- **Sequential scaling**: ThinkDeeper with varying thinking token budgets"
        )
    ...
    if is_default_test_time:
        ...
        if all(
            ...
            for metric in ["avg@5", "pass@5", "maj@5", "genselect@5"]
        ):
            ...
            report.append(
                f"- **genselect@5** (quality-based selection): {genselect5_acc:.2f}%"
            )
            ...
            if avg5_acc > 0:
                ...
                report.append(
                    f"- pass@5: {'+' if pass_improvement > 0 else ''}{pass_improvement:.1f}%"
                )
                report.append(
                    f"- maj@5: {'+' if maj_improvement > 0 else ''}{maj_improvement:.1f}%"
                )
                report.append(
                    f"- genselect@5: {'+' if genselect_improvement > 0 else ''}{genselect_improvement:.1f}%"
                )
            ...
            if pass5_acc > avg5_acc:
                ...
                report.append(
                    f"- This indicates {'high' if variance_ratio > 50 else 'moderate' if variance_ratio > 20 else 'low'} variance in response quality"
                )
...
def main():
    ...
    if args.test_time_compute:
        ...
        if args.approaches:
            ...
            approaches_config = [
                a for a in TEST_TIME_COMPUTE_APPROACHES if a[0] in args.approaches
            ]
    elif args.approaches:
        # Specific approaches requested - check all available approach lists
        all_available_approaches = (
            APPROACHES + TEST_TIME_COMPUTE_APPROACHES + DEFAULT_TEST_TIME_COMPUTE
        )
        approaches_config = []
        for requested_approach in args.approaches:
            ...
            if not found:
                logger.warning(
                    f"Approach '{requested_approach}' not found in any configuration"
                )
        ...
        logger.info(
            "Using default test-time compute evaluation (avg@5, pass@5, maj@5, genselect@5)"
        )
    ...
    for approach_name, description, extra_body_params in approaches_config:
        ...
        try:
            ...
            save_results(
                metrics, detailed_results, args.model, approach_name, args.output_dir
            )
    ...
    is_test_time = args.test_time_compute or (
        not args.approaches and approaches_config == DEFAULT_TEST_TIME_COMPUTE
    )
