...
class LogProbsCalculator:
    ...
    def _get_top_alternatives(
        self, logits: torch.Tensor, actual_token_id: int, num_alternatives: int
    ) -> Dict[str, float]:
        ...
        with torch.no_grad():
            ...
            for pos in range(sequence_length - 1):  # -1 because we look at next token
                ...
                top_logprobs = self._get_top_alternatives(
                    current_logits, next_token_id, num_alternatives
                )
...
def should_use_mlx(model_id: str) -> bool:
    ...
    for pattern in problematic_models:
        if pattern.lower() in model_lower:
            logger.warning(
                f"Model {model_id} detected as potentially problematic with MPS backend"
            )
...
def suggest_mlx_alternative(model_id: str) -> str:
    ...
    return mlx_alternatives.get(
        model_id, f"mlx-community/{model_id.split('/')[-1]}-4bit"
    )
...
class MLXInferencePipeline:
    ...
    def generate(
        ...
        for _ in range(num_return_sequences):
            try:
                logger.debug(
                    f"Generating with MLX: max_tokens={max_tokens}, temp={temperature}"
                )
                ...
                logger.error(
                    f"MLX generation parameters: max_tokens={max_tokens}, temp={temperature}, top_p={top_p}"
                )
                responses.append("")
    ...
    def process_batch(
        ...
        if len(system_prompts) != len(user_prompts):
            raise ValueError(
                f"Number of system prompts ({len(system_prompts)}) must match user prompts ({len(user_prompts)})"
            )
                ...
                for _ in range(n):
                    try:
                        ...
                        if isinstance(response, str):
                            ...
                            token_count = (
                                len(response) if hasattr(response, "__len__") else 0
                            )
                        token_counts.append(token_count)

                    except Exception as e:
                        logger.error(
                            f"Error generating response for batch item {i+1}: {e}"
                        )
    ...
    def _batch_generate(
        self, input_ids, attention_mask, generation_params: Dict
    ) -> List[str]:
        """
...
class MLXManager:
    ...
    def __init__(self, cache_manager):
        ...
        if self.available:
            ...
            logger.debug(
                "MLX manager not available - requires Apple Silicon and mlx-lm"
            )
        ...
        if self.optimized_attention is not None and hidden_states.device.type == "cuda":
            ...
            try:
                attn_output = self.optimized_attention(
                    q, k, v, attn_mask=attention_mask, causal=causal, scale=self.scale
                )
            except Exception as e:
                print(
                    f"Optimized attention failed, falling back to linear attention: {e}"
                )
...
class PromptCache:
    ...
    def get_cached_response(
        ...
        if signature in self.cache:
            ...
            if (
                abs(cached_item["temperature"] - temperature) < 0.1
                and abs(cached_item["top_p"] - top_p) < 0.1
            ):
    ...
    def add_to_cache(
        ...
        self.cache[signature] = {
            ...
            "top_p": top_p,
            "timestamp": (
                torch.cuda.current_timestamp() if torch.cuda.is_available() else 0
            ),
        }
...
class DynamicTemperature:
    ...
    def get_optimal_temperature(
        self, prompt: str, tokenizer: AutoTokenizer, base_temperature: float
    ) -> float:
...
class CacheManager:
        ...
        with self._lock:
            if not self._initialized:
                ...
                self.model_adapter_map = (
                    {}
                )  # Maps model ID to list of loaded adapter IDs
    ...
    def get_or_load_model(self, model_key: str, loader_fn) -> Tuple[Any, Any]:
        ...
        with self._lock:
            ...
            self.model_adapter_map[model_key] = (
                []
            )  # Initialize empty adapter list for new model
    ...
    def _cleanup_caches(self):
        ...
        orphaned_adapters = [
            key for key in self.adapter_cache.keys() if key not in valid_cache_keys
        ]
...
class DeviceManager:
    def __init__(self):
        ...
        self.device_stats = {
            device: {"memory_used": 0, "active_models": 0}
        ...
        if cuda_devices:
            ...
            for device in cuda_devices:
                ...
                free_memory = torch.cuda.get_device_properties(
                    idx
                ).total_memory - torch.cuda.memory_allocated(idx)
        ...
        def _load_model():
            ...
            if "cuda" in device:
                ...
                try:
                    ...
                    logger.info(
                        "Flash Attention 2 is not installed - falling back to default attention"
                    )
                ...
                if "gemma" in model_id.lower():
                    ...
                    logger.info(
                        "Using MPS device with float32 for Gemma model (float16 causes NaN)"
                    )
                else:
                    model_kwargs["torch_dtype"] = torch.float16
                ...
                if hasattr(torch.cpu, "has_fp16") and torch.cpu.has_fp16:
                    ...
                    model_kwargs["torch_dtype"] = torch.float32
                    logger.info(
                        "Using CPU device with float32 precision - FP16 not supported"
                    )

            # Load model with configured optimizations
            try:
                model = AutoModelForCausalLM.from_pretrained(
                    model_id, token=os.getenv("HF_TOKEN"), **model_kwargs
                )
            except Exception as e:
            ...
            logger.info(
                f"Model loaded successfully with dtype: {model_kwargs['torch_dtype']}"
            )
            ...
            if (
                quantize
                and "cuda" in device
                and model_kwargs["torch_dtype"] == torch.float32
            ):
...
class LoRAManager:
    ...
    def validate_adapter(self, adapter_id: str) -> bool:
        ...
        try:
            config = PeftConfig.from_pretrained(
                adapter_id, trust_remote_code=True, token=os.getenv("HF_TOKEN")
            )
            return True
        except Exception as e:
    ...
    def load_adapter(
        ...
        return self.cache_manager.get_or_load_adapter(
            model_key, adapter_id, _load_adapter
        )
    ...
    def set_active_adapter(self, model: PeftModel, adapter_id: str = None) -> bool:
        ...
        if adapter_id in available_adapters:
            ...
            logger.warning(
                f"Requested adapter {adapter_id} not loaded. Available adapters: {available_adapters}"
            )
            return False
...
class InferencePipeline:
    def __init__(
        ...
        try:
            self.base_model, self.tokenizer = self.model_manager.load_base_model(
                model_config.base_model_id, quantize=model_config.quantization_bits == 4
            )

            self.tokenizer = self.setup_tokenizer(self.tokenizer)

            if self.base_model.get_input_embeddings().num_embeddings != len(
                self.tokenizer
            ):
    ...
    def setup_tokenizer(self, tokenizer: AutoTokenizer) -> AutoTokenizer:
        ...
        logger.debug(
            f"  c. Special tokens: PAD={tokenizer.pad_token_id}, "
    ...
    def setup_efficient_attention(self):
        ...
        if hasattr(self.current_model, "config") and hasattr(
            self.current_model.config, "hidden_size"
        ):
            hidden_size = self.current_model.config.hidden_size
            num_attention_heads = self.current_model.config.num_attention_heads
            self.efficient_attention = MemoryEfficientAttention(
                hidden_size, num_attention_heads
            )

            # Monkey patch attention computation if possible
            if hasattr(self.current_model, "encoder") and hasattr(
                self.current_model.encoder, "layer"
            ):
    ...
    def _find_optimal_batch_size(
        self, initial_batch_size: int = 1, max_batch_size: int = 128
    ) -> int:
    ...
    def optimize_generation_params(self, prompt: str) -> Dict[str, Any]:
        ...
        if self.model_config.dynamic_temperature:
            base_params["temperature"] = (
                self.dynamic_temperature.get_optimal_temperature(
        ...
        return StoppingCriteriaList(
            [
                StopSequenceCriteria(
                    self.tokenizer, stop_sequences, input_length=input_length
                )
            ]
        )
        ...
        for i in range(0, len(formatted_prompts), self.optimal_batch_size):
            ...
            if self.model_config.enable_prompt_caching:
                ...
                for idx, prompt in enumerate(batch_prompts):
                    ...
                    top_p = (
                        generation_params.get("top_p", self.model_config.top_p)
                        if generation_params
                        else self.model_config.top_p
                    )

                    cached_response = (
                        self.cache_manager.prompt_cache.get_cached_response(
                            prompt, temp, top_p
                        )
                    )
            ...
            if batch_prompts:  # If there are any uncached prompts
                ...
                base_params = {
                    "max_new_tokens": (
                        generation_params.get("max_new_tokens", 4096)
                        if generation_params
                        else self.model_config.max_new_tokens
                    ),
                    "do_sample": (
                        generation_params.get("temperature", 1.0) > 0
                        if generation_params
                        else self.model_config.do_sample
                    ),
                    "temperature": (
                        generation_params.get("temperature", 1.0)
                        if generation_params
                        else self.model_config.temperature
                    ),
                    "top_p": (
                        generation_params.get("top_p", 1.0)
                        if generation_params
                        else self.model_config.top_p
                    ),
                ...
                if generation_params:
                    if generation_params.get("presence_penalty", 0) != 0:
                        base_params["presence_penalty"] = generation_params[
                            "presence_penalty"
                        ]
                    if generation_params.get("frequency_penalty", 0) != 0:
                        base_params["repetition_penalty"] = (
                            1.0 + generation_params["frequency_penalty"]
                        )
                ...
                if generation_params and generation_params.get("stop_sequences"):
                    base_params["stopping_criteria"] = self._create_stopping_criteria(
                        generation_params["stop_sequences"], input_lengths
                    )
                ...
                if self.model_config.enable_prompt_caching:
                    for prompt, response in zip(
                        batch_prompts, batch_responses[::n]
                    ):  # Only cache first response of each input
                ...
                if uncached_indices:
                    ...
                    for original_idx in range(
                        len(formatted_prompts[i : i + self.optimal_batch_size])
                    ):
            ...
            def create(
                ...
                **kwargs,
                ...
                if active_adapter is not None:
                    ...
                    pipeline.lora_manager.set_active_adapter(
                        pipeline.current_model, active_adapter
                    )
                ...
                try:
                    ...
                    if decoding:
                        ...
                        if (
                            isinstance(pipeline, MLXInferencePipeline)
                            and decoding in mlx_unsupported_decodings
                        ):
                            logger.warning(
                                f"{decoding} is not supported for MLX models. Falling back to standard generation."
                            )
                            decoding = None
                    ...
                    if decoding:
                        ...
                        if decoding == "cot_decoding":
                            ...
                            pipeline.current_model = pipeline.current_model.to(
                                torch.float32
                            )
                            ...
                            try:
                                ...
                                if seed is not None:
                                    ...
                                    generator.manual_seed(
                                        1337
                                    )  # Default seed as in original implementation
                                ...
                                completion_tokens = len(
                                    pipeline.tokenizer.encode(result)
                                )
                            ...
                            thinkdeeper_config = get_effort_profile(
                            ...
                            custom_config = {
                                "min_thinking_tokens": (
                                    min_thinking_tokens
                                    if min_thinking_tokens is not None
                                    else thinkdeeper_config["min_thinking_tokens"]
                                ),
                                "max_thinking_tokens": (
                                    max_thinking_tokens
                                    if max_thinking_tokens is not None
                                    else thinkdeeper_config["max_thinking_tokens"]
                                ),
                                "max_thoughts": (
                                    max_thoughts
                                    if max_thoughts is not None
                                    else thinkdeeper_config["max_thoughts"]
                                ),
                                "thought_switch_tokens": (
                                    thought_switch_tokens
                                    if thought_switch_tokens
                                    else thinkdeeper_config["thought_switch_tokens"]
                                ),
                                "prefill": (
                                    prefill
                                    if prefill
                                    else thinkdeeper_config["prefill"]
                                ),
                            ...
                            if isinstance(pipeline, MLXInferencePipeline):
                                ...
                                )  # thinking + response buffer
                                adjusted_max_tokens = max(
                                    user_max_tokens, total_tokens_needed
                                )

                                # Add max_tokens to thinkdeeper config
                                thinkdeeper_config_with_tokens = (
                                    thinkdeeper_config.copy()
                                )
                                thinkdeeper_config_with_tokens["max_tokens"] = (
                                    adjusted_max_tokens
                                )

                                logger.debug(
                                    f"ThinkDeeper tokens: user={user_max_tokens}, thinking={max_thinking_tokens}, adjusted={adjusted_max_tokens}"
                                )
                            ...
                            deepconf_config = {
                                ...
                                "max_tokens_per_trace": kwargs.get(
                                    "max_tokens_per_trace", 4096
                                ),
                                "temperature": temperature,
                                "confidence_metric": kwargs.get(
                                    "confidence_metric", "average_confidence"
                                ),
                            ...
                            raise ValueError(
                                f"Unknown specialized decoding approach: {decoding}"
                            )
                    ...
                    response_dict = {
                        ...
                        "choices": [
                            ...
                            for idx, (response, logprob_result) in enumerate(
                                zip(responses, logprobs_results)
                            )
                        ],
                        "usage": {
    ...
    class Models:
        ...
        def list(self):
            ...
            try:
                ...
                for model in models:
                    if (
                        "pipeline_tag" in model
                        and model["pipeline_tag"] == "text-generation"
                    ):
                        model_list.append(
                            {
                                "id": model["id"],
