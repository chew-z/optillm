...
class ThinkDeeperProcessor:
    def __init__(self, config: Dict[str, Any], tokenizer, model):
        ...
        for phrase, sequence in zip(
            self.config["thought_switch_tokens"], self.thought_switch_sequences
        ):
    ...
    def reasoning_effort(self, messages) -> str:
        ...
        logger.debug(
            f"Final response length: {len(full_response)} chars, Total thoughts: {self.thought_count}, Thinking tokens: {n_thinking_tokens}"
        )
