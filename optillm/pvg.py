...
Ensure that the Score is a single number between 0 and 10, and the Explanation is on a new line."""
    ...
    for i, solution in enumerate(solutions):
        messages = [
            ...
            {
                "role": "user",
                "content": f"Problem: {initial_query}\n\nSolution: {solution}",
            },
        ]
        provider_request = {
            "model": model,
        ...
        if score_match:
            try:
                ...
                scores.append(0)
                logger.warning(
                    f"Failed to parse score for solution {i+1}. Setting score to 0."
                )
        else:
            scores.append(0)
...
def inference_time_pv_game(
    ...
    global pvg_completion_tokens
    logger.info(
        f"Starting inference-time PV game with {num_rounds} rounds and {num_solutions} solutions per round"
    )
    ...
    for round in range(num_rounds):
        ...
        if round_best_solution[1] > best_score:
            ...
            logger.info(
                f"New best solution found in round {round + 1} with score {best_score}"
            )
        else:
            logger.debug(
                f"No improvement in round {round + 1}. Best score remains {best_score}"
            )
