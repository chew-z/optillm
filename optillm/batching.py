...
class RequestBatcher:
    ...
    def _validate_batch_compatibility(self, requests: List[BatchRequest]) -> None:
        ...
        if len(approaches) > 1:
            raise BatchingError(
                f"Cannot batch different optillm approaches: {approaches}"
            )
                ...
                try:
                    ...
                    if batch:
                        if self.enable_logging:
                            ...
                            logger.info(
                                f"Processing batch of {len(batch)} requests for {queue_key} "
    ...
    def _process_batch(self, batch: List[BatchRequest]) -> None:
        ...
        try:
            ...
            if len(responses) != len(batch):
                raise BatchingError(
                    f"Processor returned {len(responses)} responses for {len(batch)} requests"
                )
