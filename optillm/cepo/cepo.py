...
class CepoConfig:
    ...
    bestofn_rating_type: Literal[
        "absolute", "pairwise", "majority"
    ...
    planning_temperature_step1: (
        float  # temperature for generator in step 1 of planning stage
    )
    planning_temperature_step2: (
        float  # temperature for generator in step 2 of planning stage
    )
    planning_temperature_direct_resp: float  # temperature for generator after step 2 if planning fails and answer directly
    planning_temperature_step3: (
        float  # temperature for generator in step 3 of planning stage
    )
    planning_temperature_step4: (
        float  # temperature for generator in step 4 of planning stage
    )
    planning_max_tokens_step1: (
        int  # maximum number of tokens in step 1 of planning stage
    )
    planning_max_tokens_step2: (
        int  # maximum number of tokens in step 2 of planning stage
    )
    planning_max_tokens_direct_resp: float  # maximum number of tokens after step 2 if planning fails and answer directly
    planning_max_tokens_step3: (
        int  # maximum number of tokens in step 3 of planning stage
    )
    planning_max_tokens_step4: (
        int  # maximum number of tokens in step 4 of planning stage
    )
    use_plan_diversity: bool  # whether to use plan diversity
    use_reasoning_fallback: (
        bool  # whether to fallback to lower levels of reasoning when higher level fails
    )
...
def llm_call(
    client: Any, provider_request: dict, cepo_config: CepoConfig
) -> tuple[str, str, int]:
    """
    ...
    """
    retries = (
        cepo_config.num_of_retries + 1
    )  # total attempts = retries + 1 initial call
    for attempt in range(retries):
        try:
            response_object = client.chat.completions.create(
                stream=False, **provider_request
            )
            response_text, finish_reason = extract_llm_response(response_object)
            completion_tokens = (
                getattr(getattr(response_object, "usage", None), "completion_tokens", 0)
                or 0
            )
...
def llm_call_reason_effort_fallback(
    ...
    for effort in reasoning_effort_levels:
        try:
            ...
            print(
                "400/500 persisted after retries at reasoning effort",
                effort,
                "→ degrading",
            )
            continue
...
def generate_completion(
    ...
    def generate_single_plan(i):
        ...
        if cepo_config.planning_max_tokens_step1 != 0:
            ...
            if finish_reason == "length":
                return (
                    ...
                    {f"messages_planning_{i}_rejected_due_to_length": messages},
                )
        ...
        if cepo_config.planning_max_tokens_step1 != 0:
            ...
            messages.append(
                {
                    "role": "user",
                    "content": "Can you execute the above plan step-by-step to produce the final answer. Be extra careful when executing steps where your confidence is lower. /think",
                }
            )
        else:
            messages.append(
                {
                    "role": "user",
                    "content": f"Can you solve this problem step-by-step to produce the final answer? Here is the question:\n{question_only}\nRead the question again:\n\n{question_only} /think",
                }
            )
    ...
    with ThreadPoolExecutor(max_workers=cepo_config.planning_m) as executor:
        futures = [
            executor.submit(generate_single_plan, i)
            for i in range(cepo_config.planning_m)
        ]
    ...
    if not plans:
        ...
        response, finish_reason, completion_tokens = llm_call_reason_effort_fallback(
            client=client, provider_request=provider_request, cepo_config=cepo_config
        )
        local_completion_tokens += completion_tokens
    ...
    content = (
        f"Can you review your last {len(plans)} responses and identify any inconsistency between them. After that, can you address "
    ...
    if cepo_config.planning_max_tokens_step4 != 0:
        ...
        messages = [
            ...
            {
                "role": "user",
                "content": f"Here's my final solution: {final_solution}\n\nNow {content}",
            },
        ]
...
def generate_approaches(
    ...
    approaches = []
    content = (
        f'To answer the question: "{question_only}", please propose {num_approach} different high-level approaches to solve the problem. '
    ...
    while retries < max_retry:
        try:
            ...
            cleaned_completion = (
                completion.replace("\\", "\\\\").replace("json", "").replace("```", "")
            )
            ...
            print(e)
            print(
                f"Parsing Error when generating diverse approaches, retrying... ({retries + 1}/{max_retry})"
            )
...
def generate_n_completions(
    ...
    def run_single_completion(i):
        ...
        response_i, completion_tokens_i, cb_log_i = generate_completion(
            system_prompt,
            initial_query,
            client,
            model,
    ...
    with ThreadPoolExecutor(max_workers=cepo_config.bestofn_n) as executor:
        futures = [
            executor.submit(run_single_completion, i)
...
def rate_completions_absolute(
    ...
    completion_tokens = 0
    rating_prompt = (
        "Please act as an impartial judge and evaluate the accuracy of the response provided by an AI assistant to "
    ...
    for i, completion in enumerate(completions):
        ...
        rating_response = (
            match.group(1) if match else "-1"
        )  # parsing error results in a rating of -1
...
def rate_completions_pairwise(
    ...
    completion_tokens = 0
    rating_prompt = (
        "Please act as an impartial judge and compare the quality of the two responses provided by the AI assistant "
    ...
    for pair in pairs:
        ...
        if match:
            ...
            try:
                ...
                ratings[
                    pair[0]
                ] += 1  # if parsing unsuccessful, default to the first response
        else:
            ratings[
                pair[0]
            ] += 1  # if parsing unsuccessful, default to the first response
...
def cepo(
    ...
    completions, completion_tokens_planning, cb_log = generate_n_completions(
        system_prompt, initial_query, client, model, cepo_config, request_id
    )  # cb_log is a dictionary for debugging purposes
    completions = [
        c for c in completions if c
    ]  # safeguard in case completion is None (observed with GPT OSS)
    ...
    if cepo_config.bestofn_rating_type == "absolute":
        best_completion, completion_tokens_rating, cb_log = rate_completions_absolute(
            system_prompt,
            initial_query,
            client,
            rating_model,
        ...
        best_completion, completion_tokens_rating, cb_log = rate_completions_pairwise(
            system_prompt,
            initial_query,
            client,
            rating_model,
