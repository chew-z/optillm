...
class MLXThinkDeeperProcessor:
    def __init__(self, config: Dict[str, Any], tokenizer, model):
        ...
        self.max_sequence_length = (
            max(len(seq) for seq in self.thought_switch_sequences)
            if self.thought_switch_sequences
            else 5
        )

        # Track total tokens for budget management
        self.total_tokens_generated = 0
        self.max_total_tokens = config.get(
            "max_tokens", 8192
        )  # Default to 8192 if not specified
    ...
    def reasoning_effort(self, messages) -> str:
        ...
        max_chunk_size = (
            150  # Increase chunk size - MLX may work better with larger chunks
        )
        consecutive_empty_chunks = 0
        max_empty_chunks = 3  # Allow up to 3 consecutive empty chunks before stopping

        while (
            n_thinking_tokens < self.config["max_thinking_tokens"]
            and self.thought_count < self.config["max_thoughts"]
            and self.total_tokens_generated < self.max_total_tokens - 512
        ):  # Reserve 512 tokens for final response
            try:
                ...
                chunk_response = self._generate_chunk(
                    ...
                    max_tokens=min(
                        max_chunk_size,
                        self.config["max_thinking_tokens"] - n_thinking_tokens,
                    ),
                    temperature=0.6,
                )
                ...
                if not chunk_response or chunk_response.strip() == "":
                    ...
                    max_chunk_size = min(
                        max_chunk_size + 50, 300
                    )  # Increase chunk size more aggressively
                    continue
                else:
                    ...
                    if n_thinking_tokens < self.config["min_thinking_tokens"]:
                        ...
                        if after_end.strip():
                            ...
                            conclusion = self._generate_chunk(
                                current_prompt, max_tokens=200, temperature=0.3
                            )
                            if conclusion:
                ...
                if len(response_chunks) > 100:
                    logger.warning(
                        "Too many chunks generated, stopping to avoid infinite loop"
                    )
                    break
        ...
        if (
            ...
            while (
                n_thinking_tokens < self.config["min_thinking_tokens"]
                and self.thought_count < self.config["max_thoughts"]
            ):
                ...
                additional_thinking = self._generate_chunk(
                    ...
                    max_tokens=min(
                        200,
                        self.config["min_thinking_tokens"] - n_thinking_tokens + 100,
                    ),
                    temperature=0.6,
                )
        ...
        logger.debug(
            f"MLX Final response length: {len(full_response)} chars, Thinking tokens: {n_thinking_tokens}"
        )
    ...
    def _generate_chunk(self, prompt: str, max_tokens: int, temperature: float) -> str:
        ...
        try:
            ...
            sampler = make_sampler(
                temp=temperature, top_p=0.95, top_k=20, min_p=0.0, min_tokens_to_keep=3
            )
            ...
            actual_max_tokens = max(
                max_tokens, 30
            )  # At least 30 tokens for better generation
