...
class MARSVerifier:
    ...
    def verify_solutions(self, request_id: str = None) -> Dict[str, Any]:
        ...
        logger.info(
            f"Starting verification process with {self.verification_threshold}-pass threshold"
        )
        ...
        for solution in solutions:
            ...
            verification_summary["total_verifications"] += solution_verification[
                "verification_count"
            ]
        ...
        verification_summary["consensus_reached"] = len(
            verified_solutions
        ) >= self.config.get("consensus_threshold", 2)

        logger.info(
            f"Verification complete: {verification_summary['solutions_verified']} solutions verified"
        )
        return verification_summary

    async def verify_solutions_parallel(
        self, request_id: str = None, executor: ThreadPoolExecutor = None
    ) -> Dict[str, Any]:
        """Run comprehensive verification on all solutions in workspace with parallel execution"""
        logger.info(
            f"Starting parallel verification process with {self.verification_threshold}-pass threshold"
        )
        ...
        async def verify_solution_async(solution: AgentSolution):
            ...
            try:
                result = await loop.run_in_executor(
                    executor, self._verify_single_solution, solution, request_id
                )
                return result
            except Exception as e:
                logger.error(
                    f"Verification failed for solution from agent {solution.agent_id}: {str(e)}"
                )
                return {
        ...
        logger.info(
            f"Parallel verification complete: {verification_summary['solutions_verified']} solutions verified"
        )
        return verification_summary

    def _verify_single_solution(
        self, solution: AgentSolution, request_id: str = None
    ) -> Dict[str, Any]:
        ...
        for attempt in range(max_verification_attempts):
            ...
            try:
                ...
                if verification.assessment == "CORRECT":
                    ...
                    logger.info(
                        f"Verification pass {consecutive_passes}/{self.verification_threshold}"
                    )

                    # Check if we've reached the threshold
                    if consecutive_passes >= self.verification_threshold:
                        logger.info(
                            f"Solution from agent {solution.agent_id} passed {self.verification_threshold}-pass verification"
                        )
                        break
                else:
    ...
    def _select_verifier_agent(self, solution_agent_id: int) -> MARSAgent:
        ...
        available_agents = [
            agent for agent in self.agents if agent.agent_id != solution_agent_id
        ]
        ...
        if len(available_agents) > 1:
            ...
            solution_agent = next(
                (a for a in self.agents if a.agent_id == solution_agent_id), None
            )
            if solution_agent:
                solution_temp = solution_agent.temperature
                available_agents.sort(
                    key=lambda a: abs(a.temperature - solution_temp), reverse=True
                )
    ...
    def iterative_improvement(self, request_id: str = None) -> Dict[str, Any]:
        ...
        unverified_solutions = [
            s for s in self.workspace.solutions if not s.is_verified
        ]
        ...
        for solution in unverified_solutions:
            if solution.verification_results:
                ...
                if latest_verification["assessment"] in ["INCORRECT", "INCOMPLETE"]:
                    ...
                    original_agent = next(
                        (a for a in self.agents if a.agent_id == solution.agent_id),
                        None,
                    )

                    if original_agent:
                        try:
                            improved_solution, reasoning_tokens = (
                                original_agent.improve_solution(
                                    problem=self.workspace.problem,
                            ...
                            logger.info(
                                f"Improved solution from agent {solution.agent_id}"
                            )

                        except Exception as e:
                            logger.error(
                                f"Failed to improve solution from agent {solution.agent_id}: {str(e)}"
                            )
    ...
    async def iterative_improvement_parallel(
        ...
        for solution in unverified_solutions:
            if solution.verification_results:
                ...
                if latest_verification["assessment"] in ["INCORRECT", "INCOMPLETE"]:
                    ...
                    if original_agent:
                        improvable_solutions.append(
                            (solution, original_agent, latest_verification)
                        )
        ...
        logger.info(
            f"Parallel improvement complete: {improvement_summary['solutions_improved']} solutions improved"
        )
        return improvement_summary
    ...
    def final_consensus_check(self) -> bool:
        ...
        if has_consensus:
            logger.info(
                f"Consensus reached with {len(verified_solutions)} verified solutions"
            )

            # Log the consensus solutions for analysis
            for solution in verified_solutions:
                logger.info(
                    f"Verified solution from agent {solution.agent_id} (score: {solution.verification_score:.2f})"
                )
        else:
            logger.info(
                f"No consensus: only {len(verified_solutions)} solutions verified (need {consensus_threshold})"
            )
