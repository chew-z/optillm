...
async def _run_mars_parallel(
    ...
    logger.info(
        f"📝 PROBLEM: {initial_query[:200]}{'...' if len(initial_query) > 200 else ''}"
    )
    ...
    if request_config and "mars_config" in request_config:
        ...
        logger.info(
            f"⚙️  CONFIG: Applied mars_config overrides: {list(mars_config.keys())}"
        )
    ...
    max_workers = max(
        ...
        config["num_agents"]
        * min(2, config["verification_passes_required"]),  # For verification
    )
    ...
    try:
        ...
        for i, temp in enumerate(temperatures):
            ...
            logger.info(
                f"🤖 AGENTS:   Agent {i}: temp={temp}, effort={effort}, max_tokens={config['max_tokens']}"
            )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            logger.info(
                f"📊 PHASE 1: Multi-Agent Exploration - Starting parallel generation with {config['num_agents']} agents"
            )
            ...
            logger.info(
                f"📊 PHASE 1: Completed in {phase_times['exploration']:.2f}s - Generated {len(workspace.solutions)} solutions, {exploration_tokens} reasoning tokens"
            )
            ...
            if config.get("enable_aggregation", True):
                ...
                aggregation_tokens, aggregation_summary = (
                    await aggregator.run_aggregation_loops(
                ...
                logger.info(
                    f"📊 PHASE 2a: Completed in {phase_times['aggregation']:.2f}s - {aggregation_summary}, {aggregation_tokens} reasoning tokens"
                )
            ...
            if config.get("enable_strategy_network", True):
                ...
                if config.get("strategy_extraction_enabled", True):
                    extracted_strategies = (
                        await strategy_network.extract_strategies_from_solutions(
                            workspace, request_id, executor
                        )
                    )
                    ...
                    if (
                        config.get("cross_agent_enhancement", True)
                        and extracted_strategies
                    ):
                        strategy_sharing_summary = (
                        ...
                        strategy_insights = (
                            strategy_network.get_strategy_insights_summary()
                        )
                        phase_times["strategy_network"] = time.time() - phase_start
                        logger.info(
                            f"📊 PHASE 2b: Completed in {phase_times['strategy_network']:.2f}s - {strategy_insights}"
                        )

            # Phase 3: Verification System (parallel)
            phase_start = time.time()
            logger.info(
                f"📊 PHASE 3: Verification System - Verifying {len(workspace.solutions)} solutions"
            )
            verifier = MARSVerifier(agents, workspace, config)
            verification_summary = await verifier.verify_solutions_parallel(
                request_id, executor
            )
            phase_times["verification"] = time.time() - phase_start
            logger.info(
                f"📊 PHASE 3: Completed in {phase_times['verification']:.2f}s - {verification_summary}"
            )
            ...
            while (
                workspace.should_continue_iteration()
                and iteration_count < config["max_iterations"]
            ):
                iteration_count += 1
                iter_start = time.time()
                logger.info(
                    f"📊 PHASE 4: Iterative Improvement - Iteration {iteration_count}/{config['max_iterations']}"
                )

                # Improve unverified solutions (parallel)
                improvement_summary = await verifier.iterative_improvement_parallel(
                    request_id, executor
                )
                ...
                logger.info(
                    f"📊 PHASE 4: Iteration {iteration_count} completed in {iter_time:.2f}s - {improvement_summary}"
                )

                # Check for early termination
                if config["early_termination"] and workspace.has_consensus():
                    logger.info(
                        f"🎯 EARLY TERMINATION: Consensus reached after {iteration_count} iterations"
                    )
                    break
            ...
            if iteration_count > 0:
                ...
                logger.info(
                    f"📊 PHASE 4: Total improvement time: {phase_times['improvement']:.2f}s"
                )

        # Phase 5: Final Synthesis (sequential - needs all results)
        phase_start = time.time()
        logger.info(
            f"📊 PHASE 5: Final Synthesis - Processing {len(workspace.solutions)} solutions"
        )
        ...
        logger.info(
            f"📊 PHASE 5: Completed in {phase_times['synthesis']:.2f}s - Generated {len(final_solution)} char solution, {synthesis_tokens} reasoning tokens"
        )
        ...
        logger.info(
            f"🏁   Solutions: {summary['verified_solutions']}/{summary['total_solutions']} verified"
        )
        ...
        if config.get("use_thinking_tags", True):
            try:
                logger.info(
                    f"📝 ANSWER EXTRACTION: Extracting clean answer with mode '{config.get('answer_extraction_mode', 'auto')}'"
                )

                # Extract clean answer from synthesis output
                clean_answer = extract_clean_answer(
                    final_solution, mode=config.get("answer_extraction_mode", "auto")
                )

                logger.info(
                    f"📝 ANSWER EXTRACTION: Extracted {len(clean_answer)} char answer from {len(final_solution)} char synthesis"
                )
                ...
                logger.info(
                    f"📝 ANSWER EXTRACTION: Final output: {len(formatted_output)} chars (with thinking tags)"
                )
                ...
                logger.warning(
                    f"⚠️  Falling back to raw synthesis output ({len(final_solution)} chars)"
                )
                return final_solution, total_reasoning_tokens
        else:
            logger.info(
                "📝 ANSWER EXTRACTION: Thinking tags disabled, returning raw synthesis"
            )
            return final_solution, total_reasoning_tokens
        ...
        try:
            ...
            fallback_solution, fallback_tokens = fallback_agent.generate_solution(
...
async def _run_exploration_phase_parallel(
    ...
    async def generate_solution_async(agent: MARSAgent):
        ...
        try:
            solution, tokens = await loop.run_in_executor(
                executor, agent.generate_solution, workspace.problem, request_id
            )
    ...
    logger.info(
        f"Exploration phase complete: {successful_solutions} solutions generated in parallel"
    )
    return total_tokens
...
def _synthesize_final_solution(
    ...
    logger.info(
        f"🗳️  VOTING: No verified solutions found, attempting numerical voting on {len(workspace.solutions)} solutions"
    )
    ...
    logger.info(
        f"🗳️  VOTING: Starting unified answer extraction from {len(workspace.solutions)} solutions"
    )
    ...
    for i, solution in enumerate(workspace.solutions):
        ...
        if extracted_answer is not None:
            logger.info(
                f"🗳️  VOTING: Agent {solution.agent_id} extracted answer '{extracted_answer}' via unified extraction (confidence: {solution.confidence:.2f})"
            )
            ...
            for ans in answers_to_process:
                ...
                if isinstance(ans, (int, float)):
                    ...
                    extracted_answers_info.append(
                        (str(int(ans)), solution, "unified_numeric")
                    )
                    ...
                    logger.info(
                        f"🗳️  VOTING: Non-numeric answer stored for synthesis: '{ans}'"
                    )
                    ...
                    logger.info(
                        f"🗳️  VOTING: Set answer stored for synthesis: '{set_str}'"
                    )
                    break  # Use first set found

            # If no valid answer found after iterating list, log as other type
            if not any(
                isinstance(ans, (int, float, str, set))
                for ans in answers_to_process
                if isinstance(ans, str) and ans.strip()
            ):
                extracted_answers_info.append(
                    (str(extracted_answer), solution, "unified_other")
                )
                logger.info(
                    f"🗳️  VOTING: Other answer type stored for synthesis: '{extracted_answer}'"
                )
        else:
            logger.info(
                f"🗳️  VOTING: Agent {solution.agent_id} - no answer extracted via unified extraction (confidence: {solution.confidence:.2f})"
            )

    # Store extracted answers for synthesis use
    workspace._extracted_answers_info = (
        getattr(workspace, "_extracted_answers_info", []) + extracted_answers_info
    )

    # Check for majority vote
    logger.info(
        f"🗳️  VOTING: Extracted {len(numerical_answers)} numerical answers from {len(workspace.solutions)} solutions"
    )
    ...
    if len(numerical_answers) >= 2:
        ...
        for answer, count in most_common_answers:
            ...
            agents_with_answer = [
                sol.agent_id for ans, sol in numerical_answers if ans == answer
            ]
            logger.info(
                f"🗳️  VOTING:   Answer {answer}: {count}/{len(numerical_answers)} votes ({percentage:.1f}%) - Agents: {agents_with_answer}"
            )
        ...
        if count >= 2:
            ...
            matching_solutions = [
                sol for ans, sol in numerical_answers if ans == answer
            ]
            best_solution = max(matching_solutions, key=lambda s: s.confidence)

            logger.info(
                f"🎆 VOTING SUCCESS: Using majority vote answer {answer} ({count}/{len(numerical_answers)} agents agreed)"
            )
            logger.info(
                f"🎆 VOTING SUCCESS: Selected solution from agent {best_solution.agent_id} with confidence {best_solution.confidence:.2f}"
            )
            logger.info(
                f"🎆 VOTING SUCCESS: Solution length: {len(best_solution.solution)} chars"
            )
            ...
            logger.info(
                f"🗳️  VOTING: No consensus - best answer {answer} only has {count} vote(s), need 2+"
            )
    else:
        logger.info(
            f"🗳️  VOTING: Insufficient numerical answers for voting ({len(numerical_answers)} < 2)"
        )

    # If no consensus, fall back to synthesis with answer preservation
    logger.info(
        "🤔 VOTING FALLBACK: No numerical consensus found, falling back to answer-preserving synthesis"
    )
    ...
    if all_extracted:
        logger.info(
            f"🔍 EXTRACTED ANSWERS SUMMARY: Found {len(all_extracted)} extracted answers:"
        )
        for answer, solution, method in all_extracted:
            logger.info(
                f"🔍 EXTRACTED ANSWERS SUMMARY:   '{answer}' from Agent {solution.agent_id} via {method}"
            )
    else:
    ...
    input_chars = sum(
        len(sol_data["solution"]) for sol_data in synthesis_data["solutions"]
    )
    logger.info(
        f"🤝 SYNTHESIS INPUT: Processing {len(synthesis_data['solutions'])} solutions"
    )
    logger.info(f"🤝 SYNTHESIS INPUT: Total input characters: {input_chars:,}")
    logger.info(
        f"🤝 SYNTHESIS INPUT: Verification summary: {synthesis_data['verification_summary']}"
    )
    ...
    logger.info(
        f"🤝 SYNTHESIS INPUT: Using top {len(solutions_used)} solutions for synthesis:"
    )

    for i, sol_data in enumerate(solutions_used):
        logger.info(
            f"🤝 SYNTHESIS INPUT:   Solution {i+1}: Agent {sol_data['agent_id']}, {len(sol_data['solution']):,} chars, confidence {sol_data['confidence']:.2f}"
        )
    ...
    verification_text = (
        f"Verification Summary: {synthesis_data['verification_summary']}"
    )

    logger.info(
        f"🤝 SYNTHESIS INPUT: Final synthesis prompt: {synthesis_input_chars:,} characters"
    )
    ...
    if all_extracted:
        ...
        for answer, solution, method in all_extracted:
            extracted_answers_text += (
                f"- Agent {solution.agent_id}: '{answer}' (via {method})\n"
            )
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are a mathematical synthesis expert.",
                },
            ...
            timeout=300,
            extra_body={
                "reasoning": {"effort": "high"}  # High effort for final synthesis
            },
        )
        ...
        compression_ratio = (
            (output_chars / synthesis_input_chars) * 100
            if synthesis_input_chars > 0
            else 0
        )
        logger.info(
            f"🤝 SYNTHESIS PROCESSING: Input: {synthesis_input_chars:,} chars → Output: {output_chars:,} chars ({compression_ratio:.1f}% retention)"
        )
        ...
        logger.info(
            f"🤝 SYNTHESIS SUCCESS:   Output solution length: {len(final_solution)} characters"
        )
        ...
        logger.info(
            f"🤝 SYNTHESIS SUCCESS:   Solution preview: {final_solution[:200]}..."
        )
        ...
        if workspace.solutions:
            fallback_solution = max(
                workspace.solutions, key=lambda s: s.verification_score
            )
            logger.info(
                f"🚑 SYNTHESIS FALLBACK: Using fallback solution from agent {fallback_solution.agent_id}"
            )
            logger.info(
                f"🚑 SYNTHESIS FALLBACK: Solution length: {len(fallback_solution.solution):,} chars, score: {fallback_solution.verification_score:.2f}"
            )
...
def _log_solution_overview(workspace: MARSWorkspace):
    ...
    logger.info(
        f"📋 SOLUTION OVERVIEW: Analyzing {len(workspace.solutions)} solutions before synthesis"
    )
    ...
    logger.info(
        f"📋 SOLUTION OVERVIEW:   Verified solutions: {len(verified_solutions)}"
    )
    ...
    for i, solution in enumerate(workspace.solutions):
        ...
        logger.info(
            f"📋 SOLUTION OVERVIEW: Solution {i+1} (Agent {solution.agent_id}):"
        )
        ...
        logger.info(
            f"📋 SOLUTION OVERVIEW:   Verification score: {solution.verification_score:.2f}"
        )
        logger.info(
            f"📋 SOLUTION OVERVIEW:   Reasoning tokens: {solution.reasoning_tokens:,}"
        )
