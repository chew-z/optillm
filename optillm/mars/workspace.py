...
class MARSWorkspace:
    ...
    def add_solution(self, agent_solution: AgentSolution) -> str:
        ...
        logger.info(
            f"Added solution {solution_id} with {agent_solution.reasoning_tokens} reasoning tokens"
        )
        return solution_id
                ...
                for solution in self.solutions:
                    if str(solution.agent_id) == agent_id_str:
                        ...
                        verified_count = len(
                            [
                                v
                                for v in solution.verification_results
                        ...
                        verification_threshold = self.config.get(
                            "verification_passes_required", 5
                        )
                        solution.is_verified = (
                            consecutive_correct >= verification_threshold
                        )
                        break
            except (IndexError, ValueError):
                logger.warning(
                    f"Invalid solution_id format: {verification.solution_id}"
                )

        logger.info(
            f"Added verification for {verification.solution_id}: {verification.assessment}"
        )
    ...
    def should_continue_iteration(self) -> bool:
        ...
        return (
            ...
            and len(self.get_verified_solutions()) < min_verified
        )
    ...
    def _get_verification_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_confidence": sum(v.confidence for v in self.verification_results)
            / total_verifications,
        }
