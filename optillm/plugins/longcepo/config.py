...
class LongCepoConfig:
    ...
    max_output_tokens: int = (
        1024  # Max output tokens per LLM API call (except for summary generation)
    )
    max_context_window: int = 8192  # Total model context window available
    max_output_tokens_summary: int = (
        300  # Max output tokens per LLM API call (summary generation)
    )
    num_neighbor_summaries: int = (
        5  # Number of adjacent summaries from before/after in the context included in mapping stage
    )

    system_prompt: str = (
        MAPREDUCE_SYSTEM_PROMPT  # System prompt used in map/collapse/reduce stages
    )
    summary_prompt: str = (
        SUMMARY_PROMPT  # Prompt template for generating summaries in map phase
    )
    ...
    context_query_delimiter: str = (
        "<CONTEXT_END>"  # Delimiter used to split initial input into context and query
    )
    tokenizer_name: str = (
        "meta-llama/Llama-3.3-70B-Instruct"  # Tokenizer to use to determine token lengths
    )
