...
def _test_system_message_support(proxy_client, model: str) -> bool:
    ...
    try:
        ...
        if any(
            ...
            logger.info(
                f"Model {model} does not support system messages: {str(e)[:100]}"
            )
            return False
        else:
            ...
            logger.debug(
                f"System message test failed for {model}, assuming supported: {str(e)[:100]}"
            )
            return True
...
def _get_system_message_support(proxy_client, model: str) -> bool:
    ...
    with _cache_lock:
        if cache_key not in _system_message_support_cache:
            ...
            _system_message_support_cache[cache_key] = _test_system_message_support(
                proxy_client, model
            )
...
def _format_messages_for_model(
    system_prompt: str, initial_query: str, supports_system_messages: bool
) -> list:
    """
    ...
    try:
        ...
        if config_key not in _proxy_client_cache:
            ...
            _proxy_client_cache[config_key] = ProxyClient(
                config=config, fallback_client=client
            )
        else:
        ...
        if request_config:
            ...
            wrapped_approach = (
                request_config.get("proxy_wrap")
                or request_config.get("wrapped_approach")
                or request_config.get("wrap")
            )
        ...
        if wrapped_approach:
            ...
            if result is not None:
                ...
                logger.warning(
                    f"Approach/plugin '{wrapped_approach}' not found, using direct proxy"
                )
        ...
        messages = _format_messages_for_model(
            system_prompt, initial_query, supports_system_messages
        )

        if not supports_system_messages:
            logger.info(
                f"Using fallback message formatting for {model} (no system message support)"
            )
        ...
        response = proxy_client.chat.completions.create(
            model=model, messages=messages, **api_config
        )
