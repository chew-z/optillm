...
def create_comparison_prompt(
    candidates: List[str], query: str, comparison_mode: str = "batch"
) -> str:
    """
...
def parse_selection_response(response: str, num_candidates: int) -> Tuple[int, str]:
    ...
    for pattern in patterns:
        ...
        if match:
            ...
            if 0 <= selected_index < num_candidates:
                logger.info(
                    f"Selected candidate {candidate_num} using fallback pattern"
                )
    ...
    logger.warning(
        "Could not parse selection from comparison response, defaulting to first candidate"
    )
...
def run(
    ...
    comparison_temperature = config.get(
        "comparison_temperature", DEFAULT_COMPARISON_TEMPERATURE
    )
    ...
    logger.info(
        f"Generating {num_candidates} candidates with temperature={temperature}"
    )
    ...
    try:
        ...
        logger.info(
            f"Generated {len(candidates)} candidates using n parameter. Tokens: {total_tokens}"
        )
    ...
    if len(candidates) < 2:
        ...
        return (
            "Error: Could not generate sufficient candidates for selection",
            total_tokens,
        )

    # Create comparison prompt
    comparison_prompt = create_comparison_prompt(
        candidates, initial_query, comparison_mode
    )
    ...
    try:
        comparison_messages = [
            {
                "role": "system",
                "content": "You are an expert evaluator skilled at comparing and selecting high-quality responses.",
            },
