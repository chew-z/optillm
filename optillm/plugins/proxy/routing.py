...
class RoundRobinRouter(Router):
    ...
    def select(self, providers: List) -> Optional:
        ...
        logger.debug(
            f"Round-robin: Starting selection, index={self.index}, providers={[p.name for p in providers]}"
        )
        ...
        while attempts < len(self.all_providers):
            ...
            logger.debug(
                f"Round-robin: Checking provider {current_provider.name} at index {self.index}"
            )
