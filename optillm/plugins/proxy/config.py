    ...
    def _validate_config(config: Dict) -> Dict:
        ...
        for provider in config.get("providers", []):
            if "base_url" not in provider:
                raise ValueError(
                    f"Provider {provider.get('name', 'unknown')} missing base_url"
                )
            if "api_key" not in provider:
                raise ValueError(
                    f"Provider {provider.get('name', 'unknown')} missing api_key"
                )

            # Validate weight
            if provider["weight"] <= 0:
                logger.warning(
                    f"Provider {provider['name']} has invalid weight {provider['weight']}, setting to 1"
                )
                provider["weight"] = 1
            ...
            if provider.get("max_concurrent") is not None:
                if (
                    not isinstance(provider["max_concurrent"], int)
                    or provider["max_concurrent"] <= 0
                ):
                    logger.warning(
                        f"Provider {provider['name']} has invalid max_concurrent {provider['max_concurrent']}, removing limit"
                    )
        ...
        if strategy not in valid_strategies:
            logger.warning(
                f"Invalid routing strategy '{strategy}', using 'round_robin'"
            )
...
"""
    ...
    def _get_minimal_config() -> Dict:
        ...
        return {
            "providers": [],
            "routing": {"strategy": "round_robin", "health_check": {"enabled": False}},
            "timeouts": {"request": 30, "connect": 5},
            "queue": {"max_concurrent": 100, "timeout": 60},
            "monitoring": {
                "log_level": "INFO",
                "track_latency": False,
                "track_errors": True,
