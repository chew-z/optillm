...
class ApproachHandler:
    ...
    def _discover_handlers(self):
        ...
        logger.info(
            f"Discovered {len(self._approaches_cache)} approaches, "
        ...
        for name, (module_path, func_name) in approach_modules.items():
            try:
                ...
                if name == "z3":
                    ...
                    self._approaches_cache[name] = (
                        lambda s, q, c, m, **kw: solver_class(s, c, m).process_query(q)
                    )
                elif name == "rstar":
                    # Special handling for RStar
                    rstar_class = getattr(module, "RStar")
                    self._approaches_cache[name] = lambda s, q, c, m, **kw: rstar_class(
                        s, c, m, **kw
                    ).solve(q)
                elif name == "cepo":
            ...
            if any(p.kind == inspect.Parameter.VAR_KEYWORD for p in params.values()):
                ...
                if request_config:
                    ...
                    safe_kwargs = {
                        ...
                        if k
                        not in ["model", "messages", "system_prompt", "initial_query"]
                    }
