...
class Provider:
    ...
    def __init__(self, config: Dict):
        ...
        if self.max_concurrent is not None:
            ...
            logger.info(
                f"Provider {self.name} limited to {self.max_concurrent} concurrent requests"
            )
        else:
...
class ProxyClient:
    ...
    class _Completions:
        ...
        def _test_system_message_support(self, provider, model: str) -> bool:
            ...
            try:
                ...
                if any(
                    ...
                    logger.info(
                        f"Provider {provider.name} model {model} does not support system messages"
                    )
        ...
        def _format_messages_for_provider(
            self, provider, model: str, messages: list
        ) -> list:
        ...
        def _make_request_with_timeout(self, provider, request_kwargs):
            ...
            try:
                ...
                if "timeout" in str(e).lower() or "timed out" in str(e).lower():
                    raise TimeoutError(
                        f"Request to {provider.name} timed out after {self.proxy_client.request_timeout}s"
                    )
                raise e
        ...
        def create(self, **kwargs):
            ...
            if not self.proxy_client._request_semaphore.acquire(
                blocking=True, timeout=self.proxy_client.queue_timeout
            ):
                raise TimeoutError(
                    f"Request queue timeout after {self.proxy_client.queue_timeout}s - server overloaded"
                )
            ...
            try:
                ...
                healthy_providers = [
                    p for p in self.proxy_client.active_providers if p.is_healthy
                ]
                ...
                while healthy_providers:
                    available_providers = [
                        p for p in healthy_providers if p not in attempted_providers
                    ]
                    ...
                    logger.info(
                        f"Router selected provider: {provider.name if provider else 'None'}"
                    )
                    ...
                    slot_timeout = (
                        10.0  # Wait up to 10 seconds for provider to become available
                    )
                    if not provider.acquire_slot(timeout=slot_timeout):
                        logger.debug(
                            f"Provider {provider.name} at max capacity, trying next provider"
                        )
                    ...
                    try:
                        ...
                        if "messages" in request_kwargs:
                            request_kwargs["messages"] = (
                                self._format_messages_for_provider(
                        ...
                        logger.debug(
                            f"Routing to {provider.name} with {self.proxy_client.request_timeout}s timeout"
                        )
                        ...
                        logger.info(
                            f"Request succeeded via {provider.name} in {latency:.2f}s"
                        )
                        return response
                ...
                if self.proxy_client.fallback_client:
                    ...
                    try:
                        ...
                        return (
                            self.proxy_client.fallback_client.chat.completions.create(
                                **fallback_kwargs
                            )
                        )
                    except Exception as e:
