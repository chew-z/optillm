...
class MCPServer:
    ...
    async def connect_stdio(self, session: LoggingClientSession) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Server {self.server_name} initialized with capabilities: {result.capabilities}"
            )
            ...
            if hasattr(server_capabilities, "resources"):
                ...
                try:
                    ...
                    logger.debug(
                        f"Resources details: {[r.uri for r in self.resources]}"
                    )
                except McpError as e:
            ...
            logger.info(
                f"Server {self.server_name} capabilities: "
    ...
    async def connect_websocket(self) -> bool:
        ...
        if not self.config.url:
            logger.error(
                f"WebSocket transport requires URL for server {self.server_name}"
            )
            return False
        ...
        try:
            ...
            logger.error(
                f"Error connecting to WebSocket server {self.server_name}: {e}"
            )
            logger.error(traceback.format_exc())
            return False
    ...
    async def connect_stdio_native(self) -> bool:
        ...
        if not self.config.command:
            logger.error(
                f"stdio transport requires command for server {self.server_name}"
            )
            return False
        ...
        if not full_command:
            logger.error(
                f"Failed to find executable for command: {self.config.command}"
            )
            return False
        ...
        server_params = StdioServerParameters(
            command=full_command, args=self.config.args, env=merged_env
        )
    ...
    async def connect_and_discover(self) -> bool:
        ...
        logger.info(
            f"Connecting to MCP server: {self.server_name} using {self.config.transport} transport"
        )
        ...
        try:
            ...
            if success:
                self.connected = True
                logger.info(
                    f"Successfully connected to {self.server_name} via {self.config.transport}"
                )
            else:
                logger.error(
                    f"Failed to connect to {self.server_name} via {self.config.transport}"
                )
        ...
        for tool_info in self.all_tools:
            ...
            tool_entry = {
                ...
                "function": {
                    ...
                    or f"Tool {tool_name} from server {server_name}",
...
async def execute_tool_with_session(
    session: LoggingClientSession, tool_name: str, arguments: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        for content in result.content:
            if content.type == "text":
                content_results.append({"type": "text", "text": content.text})
        ...
        return {"result": content_results, "is_error": result.isError}
...
async def execute_tool_stdio(
    ...
    if not full_command:
        return {
            "error": f"Failed to find executable for command: {server_config.command}"
        }
    ...
    # Create server parameters
    server_params = StdioServerParameters(
        command=full_command, args=server_config.args, env=merged_env
    )
...
async def execute_tool_websocket(
    server_config: ServerConfig, tool_name: str, arguments: Dict[str, Any]
) -> Dict[str, Any]:
...
async def run(
    ...
    try:
        ...
        if not success:
            logger.warning(
                "Failed to connect to any MCP servers, falling back to default behavior"
            )
            response = client.chat.completions.create(
                model=model,
                messages=[
        ...
        if hasattr(response_message, "tool_calls") and response_message.tool_calls:
            logger.info(
                f"Model requested tool calls: {len(response_message.tool_calls)}"
            )
            for i, tc in enumerate(response_message.tool_calls):
                logger.debug(
                    f"Tool call {i+1}: {tc.function.name} with args: {tc.function.arguments}"
                )
            ...
            for tool_call in response_message.tool_calls:
                ...
                if "." in full_tool_name:
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Added tool result for {full_tool_name}: {json.dumps(result)[:100]}..."
                        )
                    except Exception as e:
                        logger.error(
                            f"Error processing tool call {full_tool_name}: {e}"
                        )
                        messages.append(
                            {
                                "role": "tool",
                    ...
                    messages.append(
                        {
                            ...
                            "content": json.dumps(
                                {
                                    "error": f"Invalid tool name format: {full_tool_name}. Expected format: server_name.tool_name"
                                }
                            ),
                        }
                    )
            ...
            final_response = client.chat.completions.create(
                ...
                ),  # Keep tools available in case the model wants to make additional calls
                temperature=0.7,
            )
