...
class UncertaintyRoutedCoT:
    ...
    def generate_with_uncertainty_routing(
        ...
        for sample in samples:
            ...
            sample_data.append(
                {"full_response": sample, "thinking": thinking, "answer": answer}
            )
        ...
        logger.debug(
            f"Sample answers: {[sample['answer'][:50] + '...' if len(sample['answer']) > 50 else sample['answer'] for sample in sample_data if sample['answer']]}"
        )
        ...
        if confidence_score >= confidence_threshold:
            ...
            logger.info(
                f"High confidence ({confidence_score:.3f} >= {confidence_threshold}) - using majority vote"
            )
        else:
            ...
            logger.info(
                f"Low confidence ({confidence_score:.3f} < {confidence_threshold}) - using greedy sample"
            )
    ...
    def _generate_multiple_samples(
        self, prompt: str, num_samples: int, temperature: float, top_p: float
    ) -> List[str]:
        ...
        for i in range(num_samples):
            ...
            if (
                ...
                logger.warning(
                    f"Sample {i+1}/{num_samples} truncated or empty, using empty string"
                )
                samples.append("")
            else:
    ...
    def _generate_greedy_sample(self, prompt: str) -> str:
        ...
        if (
            ...
            logger.error(
                "Greedy sample truncated or empty. Consider increasing max_tokens."
            )
    ...
    def _evaluate_confidence(self, sample_data: List[Dict[str, Any]]) -> float:
        ...
        thinking_texts = [
            sample["thinking"] for sample in sample_data if sample["thinking"]
        ]
        ...
        logger.debug(
            f"Reasoning consistency: {reasoning_consistency:.3f} (weight: 0.4)"
        )
        ...
        if confidence < 0.5:
            ...
            logger.debug(
                f"Answers found: {len(answers)}, Thinking texts: {len(thinking_texts)}"
            )
            if answers:
                logger.debug(f"Sample answers: {answers}")
            if len(answers) >= 2:
                logger.debug(
                    f"Most common answer appears {max(Counter(answers).values())} times out of {len(answers)}"
                )
    ...
    def _calculate_answer_consistency(self, answers: List[str]) -> float:
        ...
        logger.debug(
            f"Agreement ratio: {agreement_ratio:.3f} ({most_common_count}/{total_answers})"
        )
    ...
    def _calculate_reasoning_consistency(self, thinking_texts: List[str]) -> float:
        ...
        logger.debug(
            f"Reasoning similarity pairs: {[f'{s:.3f}' for s in similarities]}"
        )
    ...
    def _majority_vote_response(self, sample_data: List[Dict[str, Any]]) -> str:
        ...
        if best_sample:
            ...
            for sample in sample_data:
                if sample["answer"]:
                    norm_answer = re.sub(
                        r"[^\w\s]", "", sample["answer"].lower().strip()
                    )
                    norm_answer = re.sub(r"\s+", " ", norm_answer)
                    if norm_answer == most_common_answer:
