...
class ResearchSessionState:
    ...
    def get_or_create_session(
        self, session_id: str, headless: bool = False, timeout: int = 30
        ...
        with self._lock:
            print(
                f"🔍 Session state: {len(self._sessions)} active sessions, checking for ID: {session_id}"
            )
            ...
            if session_id in self._sessions:
                ...
                print(
                    f"📋 Found existing session for ID: {session_id}, active: {session.is_active()}, instance: {id(session)}"
                )
                if session.is_active():
                    print(
                        f"♻️  Reusing existing browser session for research ID: {session_id}"
                    )
                    return session
                else:
            ...
            print(
                f"✅ Created new session instance: {id(session)} for ID: {session_id}"
            )
...
def get_session_manager(
    session_id: str, headless: bool = False, timeout: int = 30
) -> Optional[BrowserSessionManager]:
    """
