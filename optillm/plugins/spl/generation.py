...
def classify_problem(content: str, client, model: str) -> str:
    ...
    problem_types_str = ", ".join(
        VALID_PROBLEM_TYPES[:-1]
    )  # Exclude the general_problem fallback

    try:
        messages = [
            {
                "role": "system",
                "content": PROBLEM_CLASSIFICATION_PROMPT.format(
                    problem_types=problem_types_str
                ),
            },
            {
                "role": "user",
        ...
        logger.debug(
            f"Problem classification - final response after removing thinking: '{final_response}'"
        )
        ...
        for valid_type in VALID_PROBLEM_TYPES:
            if valid_type.lower() in final_response:
                logger.info(
                    f"Classified problem as '{valid_type}' (partial match from '{final_response}')"
                )
                return valid_type

        # If still no match, return the general_problem fallback
        logger.warning(
            f"Could not match '{final_response}' to any valid problem type, using 'general_problem'"
        )
...
def generate_strategy(
    problem: str, problem_type: str, client, model: str, db: StrategyDatabase
) -> Strategy:
    """
    ...
    try:
        ...
        logger.debug(
            f"Generated strategy - final text after removing thinking: '{strategy_text}'"
        )
        ...
        logger.info(
            f"Generated new strategy for {problem_type}: ID {strategy.strategy_id}"
        )
        return strategy
...
def should_create_new_strategy(
    ...
    if (
        len(existing_strategies) >= MAX_STRATEGIES_PER_TYPE
    ):  # Using storage limit here, not inference limit
        ...
        if similar_strategy_result:
            ...
            logger.info(
                f"Found similar strategy {similar_strategy.strategy_id} with text similarity {similarity:.2f}"
            )
        ...
        if similar_examples_result:
            ...
            logger.info(
                f"Found strategy {similar_strategy.strategy_id} with similar examples, similarity {similarity:.2f}"
            )
            return False, similar_strategy
        ...
        if existing_strategies:
            ...
            logger.info(
                f"At maximum strategies for {problem_type}, updating lowest performing strategy {worst_strategy.strategy_id}"
            )
    ...
    similar_strategy_result = db.find_similar_strategy(
        problem_type, query, threshold=STRATEGY_CREATION_THRESHOLD
    )
    if similar_strategy_result:
        similar_strategy, similarity = similar_strategy_result
        logger.info(
