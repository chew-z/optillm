...
from .generation import classify_problem, generate_strategy, should_create_new_strategy
from .evaluation import (
    select_relevant_strategies,
    evaluate_strategy_effectiveness,
    refine_strategy,
)
from .utils import extract_thinking, augment_system_prompt
from .config import (
    DEFAULT_MAX_TOKENS,
...
def run_spl(
    ...
    logger.info(
        f"Found {len(existing_strategies)} existing strategies for {problem_type}"
    )
    ...
    if learning_mode:
        ...
        should_create, similar_strategy = should_create_new_strategy(
            problem_type, initial_query, existing_strategies, db
        )
        ...
        if should_create:
            ...
            new_strategy = generate_strategy(
                initial_query, problem_type, client, model, db
            )
            ...
            logger.info(
                f"Updating existing strategy {similar_strategy.strategy_id} with new example"
            )
    ...
    if learning_mode and db.metrics["total_queries"] % MAINTENANCE_INTERVAL == 0:
        ...
        merged_count = db.merge_similar_strategies(
            similarity_threshold=STRATEGY_MERGING_THRESHOLD
        )
        ...
        limited_count = db.limit_strategies_per_type(
            max_per_type=MAX_STRATEGIES_PER_TYPE
        )
    ...
    selected_strategies = select_relevant_strategies(
        initial_query, problem_type, db, learning_mode, MAX_STRATEGIES_FOR_INFERENCE
    )

    # Log the selected strategies
    for i, strategy in enumerate(selected_strategies, 1):
        logger.info(
            f"Selected strategy {i}/{MAX_STRATEGIES_FOR_INFERENCE} for inference: {strategy.strategy_id} (success rate: {strategy.success_rate:.2f})"
        )
    ...
    if not selected_strategies:
        if not existing_strategies:
            ...
            logger.info(
                f"No strategies exist for problem type '{problem_type}'. Enable learning mode with 'spl_learning=True' to create strategies."
            )
        else:
            # Strategies exist but don't meet the minimum success rate
            logger.info(
                f"Strategies exist for problem type '{problem_type}' but none meet the minimum success rate threshold of {MIN_SUCCESS_RATE_FOR_INFERENCE:.2f}."
            )
            logger.info(
                "Enable learning mode with 'spl_learning=True' to improve strategies."
            )

        # Use the original system prompt without augmentation
        logger.info(
            "Running without strategy augmentation - using base system prompt only."
        )
        ...
        logger.info(
            f"Augmented system prompt with {len(selected_strategies)} strategies (inference limit: {MAX_STRATEGIES_FOR_INFERENCE})"
        )
    ...
    try:
        ...
        if request_config:
            request_params = {
                k: v for k, v in request_config.items() if k != "spl_learning"
            }
        ...
        if thinking:
            ...
            logger.debug(
                f"Main response - final answer after removing thinking: '{final_response}'"
            )
        ...
        if learning_mode:
            if selected_strategies:
                ...
                strategy_effectiveness = evaluate_strategy_effectiveness(
                    final_response, thinking, selected_strategies, client, model
                )
                ...
                for strategy_id, effective in strategy_effectiveness.items():
                    ...
                    if strategy_id != "fallback_temporary":
                        ...
                        logger.info(
                            f"Strategy {strategy_id} effectiveness: {effective}"
                        )
                        ...
                        if (
                            effective
                            and thinking
                            and strategy_id != "fallback_temporary"
                        ):
                            db.add_reasoning_example(strategy_id, thinking)
                            logger.info(
                                f"Added reasoning example to strategy {strategy_id}"
                            )
                ...
                for strategy in selected_strategies:
                    # Skip temporary fallback strategies
                    if (
                        strategy.strategy_id != "fallback_temporary"
                        and strategy.total_attempts % 10 == 0
                        and strategy.total_attempts > 0
                    ):
                        logger.info(
                            f"Refining strategy {strategy.strategy_id} after {strategy.total_attempts} attempts"
                        )
                        refined_strategy = refine_strategy(
                            strategy,
                            initial_query,
                            final_response,
                            thinking,
                        ...
                        db.refine_strategy(
                            strategy.strategy_id, refined_strategy.strategy_text
                        )
            else:
                logger.info(
                    "No strategies to evaluate or refine - consider adding strategies for this problem type"
                )
        else:
            logger.info(
                "Strategy evaluation and refinement skipped (not in learning mode)"
            )
