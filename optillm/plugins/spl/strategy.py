...
class Strategy:
    ...
    def __init__(
        ...
        self.problem_type = (
            problem_type if problem_type in VALID_PROBLEM_TYPES else "general_problem"
        )
    ...
    def record_attempt(self, success: bool) -> None:
        ...
        self.confidence = (1 - alpha) * self.confidence + alpha * (
...
class StrategyDatabase:
    ...
    def _load(self) -> None:
        ...
        if os.path.exists(self.db_path):
            try:
                        ...
                        if strategy.strategy_id.startswith("strategy_"):
                            try:
                                ...
                                self.metrics["last_strategy_id"] = max(
                                    self.metrics["last_strategy_id"], strategy_num
                                )
                            except ValueError:
                ...
                logger.info(
                    f"Loaded {len(self.strategies)} strategies from {self.db_path}"
                )
            ...
            try:
                with open(self.metrics_path, "r") as f:
                    ...
                    if "last_strategy_id" in metrics:
                        self.metrics["last_strategy_id"] = max(
                            last_id, metrics["last_strategy_id"]
                        )
    ...
    def add_strategy(self, strategy: Strategy) -> None:
        ...
        if not exists:
            logger.info(
                f"Added first strategy for problem type: {strategy.problem_type}"
            )
        else:
            logger.info(
                f"Added additional strategy for problem type: {strategy.problem_type}"
            )
    ...
    def get_similar_strategies(
        ...
        if (
            ...
            or len(self.vectors.shape) == 0
            or self.vectors.shape[0] != len(strategy_texts)
        ):
            try:
        ...
        try:
            ...
            return [
                (self.strategies[i], float(similarities[i])) for i in sorted_indices[:n]
            ]
        except Exception as e:
    ...
    def find_similar_strategy(
        ...
        problem_type: str,
        query: str,
        threshold: float = STRATEGY_CREATION_THRESHOLD,
    ...
    def prune_strategies(
        self, min_success_rate: float = 0.3, min_attempts: int = 5
    ) -> int:
        ...
        self.strategies = [
            ...
            if s.total_attempts < min_attempts or s.success_rate >= min_success_rate
        ]
    ...
    def merge_similar_strategies(
        self, similarity_threshold: float = STRATEGY_MERGING_THRESHOLD
    ) -> int:
            ...
            while j < len(self.strategies):
                ...
                if self.strategies[i].problem_type == self.strategies[j].problem_type:
                    ...
                    if similarity >= similarity_threshold:
                        # Merge strategies
                        merged_strategy = self._merge_two_strategies(
                        ...
                        logger.info(
                            f"Merged strategies {self.strategies[i].strategy_id} and {merged_strategy.strategy_id} with similarity {similarity:.2f}"
                        )
                    else:
                        j += 1
                else:
    ...
    def _merge_two_strategies(
        ...
        merged = Strategy(
            ...
            created_at=(
                min(base.created_at, other.created_at)
                if base.created_at and other.created_at
                else base.created_at
            ),
            last_used=(
                max(base.last_used, other.last_used)
                if base.last_used and other.last_used
                else base.last_used
            ),
    ...
    def limit_strategies_per_type(
        self, max_per_type: int = MAX_STRATEGIES_PER_TYPE
    ) -> int:
        """
        ...
        if removed_count > 0:
            ...
            self._save()
            logger.info(
                f"Removed {removed_count} excess strategies to maintain max {max_per_type} per type in database (storage limit)"
            )
