                ...
                if self.parent.client_type == "openai_compatible":
                    ...
                    try:
                        ...
                        if "Cerebras" in self.parent.client.__class__.__name__:
                            ...
                            existing_http_client = getattr(
                                self.parent.client, "_client", None
                            )
                            ...
                            if "Azure" in self.parent.client.__class__.__name__:
                                ...
                                custom_client = AzureOpenAI(
                                    api_key=self.parent.client.api_key,
                                    api_version=getattr(
                                        self.parent.client, "api_version", None
                                    ),
                                    azure_endpoint=getattr(
                                        self.parent.client, "azure_endpoint", None
                                    ),
                                    azure_ad_token_provider=getattr(
                                        self.parent.client,
                                ...
                                custom_client = OpenAI(
                                    api_key=self.parent.client.api_key,
                                    base_url=getattr(
                                        self.parent.client, "base_url", None
                                    ),
                                    timeout=self.parent.timeout,
                                    max_retries=self.parent.max_retries,
                                    http_client=existing_http_client,
                        ...
                        print(
                            f"⚠️ Warning: Could not create custom client with timeout: {str(e)}"
                        )
                    ...
                    print(
                        f"ℹ️ Using original client (type: {self.parent.client.__class__.__name__}) without timeout modification"
                    )
                    return self.parent.client.chat.completions.create(**kwargs)
...
def run(
    ...
    max_iterations = config.get(
        "max_iterations", 5
    )  # Default to 5 iterations for faster results
