...
def best_of_n_sampling(
    ...
    try:
        ...
        logger.info(
            f"Generated {len(completions)} completions using fallback method. Total tokens used: {bon_completion_tokens}"
        )
    ...
    for completion in completions:
        ...
        if (
            ...
            logger.warning(
                "Rating response truncated or empty, using default rating of 0"
            )
            ratings.append(0)
        else:
            try:
