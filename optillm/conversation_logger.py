...
class ConversationLogger:
    ...
    def __init__(self, log_dir: Path, enabled: bool = False):
        ...
        if self.enabled:
            ...
            logger.info(
                f"Conversation logging enabled. Logs will be saved to: {self.log_dir}"
            )
        else:
    ...
    def start_conversation(
        self, client_request: Dict[str, Any], approach: str, model: str
    ) -> str:
        """
    ...
    def log_provider_call(
        ...
        logger.debug(
            f"Logged provider call #{len(entry.provider_calls)} for request {request_id}"
        )
...
def log_provider_call(
    ...
    if _global_logger and _global_logger.enabled:
        _global_logger.log_provider_call(
            request_id, provider_request, provider_response
        )
