...
class SteeringVectorManager:
    ...
    def load_dataset(self):
        ...
        try:
            ...
            logger.info(
                f"Found {len(self.pattern_to_vectors)} reasoning patterns: {list(self.pattern_to_vectors.keys())}"
            )
            ...
            if self.steering_vectors:
                ...
                logger.info(
                    f"First vector sample - pattern: {first_vector.get('reasoning_pattern', 'missing')}"
                )
        ...
        try:
            ...
            if isinstance(steering_vector, str):
                ...
                try:
                    ...
                    steering_vector = [
                        float(x) for x in steering_vector.strip("[]").split(",")
                    ]

            # Ensure we have a proper list
            if not isinstance(steering_vector, list):
                logger.warning(
                    f"Invalid steering vector format: {type(steering_vector)}"
                )
                return None
    ...
    def create_tokenized_contexts(self, tokenizer):
        ...
        logger.info(
            f"STEERING: Pre-tokenized {count} contexts into {len(self.tokenized_contexts)} token patterns"
        )
        ...
        logger.info(
            f"STEERING: Token pattern length distribution: {sorted(length_counts.items())}"
        )
    ...
    def get_steering_vector(
        self, context: str, match_key: Optional[str] = None
        ...
        if match_key is not None:
            ...
            for vector in self.steering_vectors:
                ...
                if vector_key == match_key:
                    logger.debug(
                        f"STEERING: Context match found for '{vector.get('pivot_token', '')}' with pattern {vector.get('reasoning_pattern', 'unknown')}"
                    )
                    return vector
                ...
                if random.random() < 0.001:  # Log a small random sample for debugging
                    logger.debug(
                        f"STEERING: Match failed - key length: {len(match_key)}, vector key length: {len(vector_key)}"
                    )
...
class SteeringHook:
    ...
    def __call__(self, module, input_tensors, output):
        ...
        try:
            ...
            if self.current_vector is not None:
                ...
                safe_strength = min(
                    max(strength, 0.1), 2.0
                )  # Limit between 0.1 and 2.0

                # Log when pattern changes
                if pattern != self.last_pattern:
                    logger.info(
                        f"STEERING: Switching to {pattern} reasoning pattern with strength {safe_strength}"
                    )
                    ...
                    if random.random() < 0.05:
                        logger.info(
                            f"STEERING: Still applying {pattern} pattern with strength {safe_strength}"
                        )
                ...
                try:
                    if isinstance(output, tuple):
                        ...
                        try:
                            ...
                            modified_hidden_states = self._apply_steering_vector(
                                hidden_states, self.current_vector, safe_strength
                            )
                            ...
                            if modified_hidden_states.shape == hidden_states.shape:
                                ...
                                logger.error(
                                    f"STEERING: Modified hidden states have wrong shape. Expected {hidden_states.shape}, got {modified_hidden_states.shape}"
                                )
                                return output
                        except Exception as e:
                            logger.error(
                                f"STEERING: Error applying steering to tuple output: {e}"
                            )
                            return output
                    else:
                        ...
                        try:
                            ...
                            return self._apply_steering_vector(
                                output, self.current_vector, safe_strength
                            )
                        except Exception as e:
                            logger.error(
                                f"STEERING: Error applying steering to direct output: {e}"
                            )
                            return output

                except Exception as e:
                    logger.error(
                        f"STEERING: Unexpected error in steering application: {e}"
                    )
                    return output
    ...
    def _apply_steering_vector(
        ...
        try:
            ...
            try:
                vector = torch.tensor(
                    vector_data, dtype=hidden_states.dtype, device=hidden_states.device
                )
            except Exception as e:
            ...
            logger.debug(
                f"STEERING: Applying {vector_type} for pattern '{pattern}' with base scaling {scaling_factor}"
            )
            ...
            if "prob_delta" in steering_vector:
                ...
                logger.debug(
                    f"STEERING: Adjusted scaling by prob_delta {prob_delta_capped} to {scaling_factor}"
                )
            ...
            logger.debug(
                f"STEERING: hidden_states shape: {hs_shape}, vector shape: {vector_shape}"
            )

            # Verify shapes are compatible
            if len(vector_shape) != 1 or vector_shape[0] != hs_shape[-1]:
                logger.error(
                    f"STEERING: Shape mismatch - hidden_states: {hs_shape}, vector: {vector_shape}"
                )
                return hidden_states
            ...
            try:
                if len(hs_shape) >= 3 and hs_shape[0] > 0 and hs_shape[1] > 0:
                    ...
                    if is_positive:
                        ...
                        hidden_states_clone[-1, -1, :] = (
                            hidden_states_clone[-1, -1, :] + safe_scaling * vector_norm
                        )
                    else:
                    ...
                    if (
                        torch.isnan(hidden_states_clone).any()
                        or torch.isinf(hidden_states_clone).any()
                    ):
                        logger.error(
                            "STEERING: NaN or inf values detected after applying vector, reverting to original"
                        )
                        return hidden_states
                else:
                    logger.error(
                        f"STEERING: Hidden states shape not suitable for steering: {hs_shape}"
                    )
                    return hidden_states
            except IndexError as e:
                logger.error(f"STEERING: IndexError when applying vector: {e}")
                logger.error(
                    f"STEERING: Indices: [-1, -1, :], tensor shape: {hidden_states.shape}"
                )
                return hidden_states
        ...
        if self.tokenizer is not None:
            ...
            if token_ids:  # Only proceed if we got tokens
                ...
                if random.random() < 0.01:
                    logger.debug(
                        f"STEERING: Token history updated, now has {len(self.token_history)} tokens"
                    )
        else:
            ...
            if len(self.context_buffer) > 500:
                ...
                logger.debug(
                    f"STEERING: Context buffer trimmed to {len(self.context_buffer)} chars"
                )
    ...
    def try_match(self):
        ...
        if (
            ...
            and hasattr(self.manager, "tokenized_contexts")
            and self.manager.tokenized_contexts
        ):
    ...
    def _try_token_match(self):
        ...
        if len(self.token_history) < 4:
            logger.debug(
                f"STEERING: Not enough tokens to match ({len(self.token_history)})"
            )
            return False
        ...
        if random.random() < 0.01:
            history_sample = (
                self.token_history[-5:]
                if len(self.token_history) >= 5
                else self.token_history
            )
            logger.debug(
                f"STEERING: Token matching with history (last {len(history_sample)} of {len(self.token_history)} tokens): {history_sample}"
            )
        ...
        if best_match["vector"] is not None:
            ...
            logger.info(
                f"STEERING: Found {match_type} token match ({best_match['match_len']}/{best_match['token_len']} tokens) for {pattern} pattern"
            )
        ...
        if len(self.token_history) >= 8 and not self.match_found:
            ...
            for tokenized_context, vector in self.manager.tokenized_contexts.items():
                ...
                if (
                    token_len >= 8
                ):  # Only try fuzzy matching for contexts with enough tokens
                    ...
                    matches = sum(
                        1 for a, b in zip(last_tokens, context_tokens) if a == b
                    )
            ...
            if best_match["vector"] is not None:
                ...
                logger.info(
                    f"STEERING: Found fuzzy match ({similarity:.2f} similarity) for {pattern} pattern"
                )
                logger.info(f"STEERING: Pivot token: '{pivot_token}'")
    ...
    def _try_text_match(self):
        ...
        match_key = (
            self.context_buffer[-100:]
            if len(self.context_buffer) >= 100
            else self.context_buffer
        )

        # Log context buffer periodically to debug
        if random.random() < 0.01:  # Log occasionally to avoid spam
            logger.debug(
                f"STEERING: Current context buffer (last 50 chars): '{self.context_buffer[-50:]}'"
            )
            logger.debug(
                f"STEERING: Matching with key (length {len(match_key)}): '{match_key[:20]}...'"
            )
        ...
        if len(match_key) >= 20:  # Only try for reasonably sized contexts
            ...
            if best_match is not None:
                ...
                logger.info(
                    f"STEERING: Found fuzzy text match ({best_similarity:.2f} similarity) for {pattern} pattern"
                )
                logger.info(f"STEERING: Pivot token: '{pivot_token}'")
                return True
...
def install_steering_hooks(
    model, manager: SteeringVectorManager, tokenizer=None
) -> List[Tuple]:
    """
    ...
    if hasattr(model, "config"):
        logger.info(
            f"STEERING: Model architecture is {model.config.architectures[0] if hasattr(model.config, 'architectures') else 'unknown'}"
        )
    ...
    if hasattr(model, "transformer"):
        ...
        if hasattr(model.model, "layers") and layer_num < len(model.model.layers):
            ...
            and layer_num < len(model.model.decoder.layers)
        ):
    ...
    if module is None:
        logger.error(
            f"STEERING: Could not find appropriate module for layer {layer_num}"
        )
        logger.error(
            "STEERING: Model structure not compatible with current hook installation logic"
        )
        return []
