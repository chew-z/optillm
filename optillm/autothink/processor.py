...
class AutoThinkProcessor:
    ...
    def __init__(
        ...
        self._start_think_token = (
            start_tokens[0] if len(start_tokens) == 1 else start_tokens[1]
        )
        ...
        self.max_sequence_length = max(
            len(seq) for seq in self.thought_switch_sequences
        )
    ...
    def _setup_steering(self):
        ...
        try:
            ...
            self.steering_hooks = install_steering_hooks(
                self.model, self.steering_manager, self.tokenizer
            )

            logger.info(
                f"STEERING: Set up steering with {len(self.steering_hooks)} hooks"
            )
    ...
    def classify_complexity(self, query: str) -> Tuple[str, float]:
        ...
        logger.info(
            f"Query classified as {complexity} with confidence {confidence:.2f}"
        )
    ...
    def is_thought_switch(self, token: int) -> bool:
        ...
        for sequence in self.thought_switch_sequences:
            if (
                len(sequence) <= len(self.current_sequence)
                and self.current_sequence[-len(sequence) :] == sequence
            ):
                return True
    ...
    def process(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            logger.info(
                f"Using token budget: {min_tokens}-{max_tokens} for {complexity} complexity"
            )
            ...
            tokens = self.tokenizer.apply_chat_template(
                thinking_messages, continue_final_message=True, return_tensors="pt"
            ...
            while True:
                ...
                force_end = (
                    n_thinking_tokens >= max_tokens
                    or self.thought_count >= self.config["max_thoughts"]
                )

                if force_end and not seen_end_think:
                    logger.debug(
                        f"Forcing end think token. Tokens: {n_thinking_tokens}, Thoughts: {self.thought_count}"
                    )
                ...
                if not seen_end_think and self.is_thought_switch(next_token):
                    ...
                    logger.debug(
                        f"Detected thought switch marker. Total thoughts: {self.thought_count}"
                    )
                ...
                if next_token == self.end_think_token:
                    ...
                    if n_thinking_tokens < min_tokens:
                        replacement = random.choice(
                            self.config["thought_switch_tokens"]
                        )
                        logger.debug(
                            f"Inserting thought transition: '{replacement}' (tokens: {n_thinking_tokens})"
                        )
                ...
                if next_token == self.model.config.eos_token_id:
                    ...
                    if seen_end_think:
                        logger.debug(
                            "Reached EOS after end think token - stopping generation"
                        )
                        ...
                        logger.debug(
                            "Reached EOS without end think token - adding end token and continuing generation"
                        )
                        response_chunks.append(
                            self.tokenizer.decode([self.end_think_token])
                        )
                        tokens = torch.tensor([[self.end_think_token]]).to(
                            tokens.device
                        )
                        seen_end_think = True
                        continue
            ...
            logger.debug(
                f"Final response length: {len(full_response)} chars, Total thoughts: {self.thought_count}"
            )
