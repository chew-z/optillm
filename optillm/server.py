...
def get_config():
    ...
    # Minimal provider env visibility without exposing secrets
    logger.debug(f"OPENAI_API_KEY set: {bool(os.environ.get('OPENAI_API_KEY'))}")
    logger.debug(
        f"AZURE_OPENAI_API_KEY set: {bool(os.environ.get('AZURE_OPENAI_API_KEY'))}"
    )
    ...
    # Cerebras, OpenAI, Azure, or LiteLLM API configuration
        ...
        masked_key = (API_KEY[:6] + "..." + API_KEY[-4:]) if API_KEY else ""
        logger.info(
            "ZAI provider selected (ZAI_API_KEY set: True, key=***%s)" % masked_key
        )
        if ZAI_BASE_URL:
            logger.debug(f"ZAI_BASE_URL: {ZAI_BASE_URL}")
        base_url = server_config["base_url"]
        if base_url != "":
    ...
    try:
        ...
        if conversation_logger and request_id:
            conversation_logger.log_error(
                request_id, f"Error in none approach: {str(e)}"
            )
...
def load_plugins():
    ...
    current_dir = (
        os.getcwd()
        if server_config.get("plugins_dir", "") == ""
        else server_config["plugins_dir"]
    )
    ...
    for plugin_dir, source in plugin_dirs:
        ...
        if not os.path.exists(plugin_dir):
            logger.debug(
                f"{source.capitalize()} plugin directory not found: {plugin_dir}"
            )
            continue
...
def get_config_path():
    ...
    package_config_dir = os.path.join(
        os.path.dirname(optillm.__file__), "cepo", "configs"
    )
    ...
    current_dir = (
        os.getcwd()
        if server_config.get("config_dir", "") == ""
        else server_config["config_dir"]
    )
...
def execute_single_approach(
    ...
    if approach in known_approaches:
        if approach == "none":
            ...
            return round_trip_optimization(
                system_prompt, initial_query, client, model, request_config, request_id
            )
        elif approach == "z3":
            ...
            return cepo(
                system_prompt, initial_query, client, model, cepo_config, request_id
            )
        elif approach == "mars":
        ...
        if is_async:
            ...
            try:
                if "request_config" in sig.parameters:
                    ...
                    result = loop.run_until_complete(
                        plugin_func(system_prompt, initial_query, client, model)
                    )
                return result
            finally:
                loop.close()
...
def execute_combined_approaches(
    ...
    for approach in approaches:
        response, tokens = execute_single_approach(
            approach, system_prompt, final_response, client, model, request_config
        )
...
async def execute_parallel_approaches(
    approaches, system_prompt, initial_query, client, model, request_config: dict = None
):
    ...
    for _ in range(n):
        if operation == "SINGLE":
            ...
            response, tokens = execute_combined_approaches(
                approaches, system_prompt, initial_query, client, model, request_config
            )
        elif operation == "OR":
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
...
def extract_optillm_approach(content):
    ...
    if match:
        ...
        content = re.sub(
            r"<optillm_approach>.*?</optillm_approach>", "", content
        ).strip()
...
def proxy():
    ...
    if max_completion_tokens is not None:
        ...
        request_config["max_tokens"] = (
            max_completion_tokens  # For backward compatibility with approaches that read max_tokens
        )
    ...
    server_config["mcts_exploration"] = data.get(
        "mcts_exploration", server_config["mcts_exploration"]
    )
    server_config["mcts_simulations"] = data.get(
        "mcts_simulations", server_config["mcts_simulations"]
    )

    system_prompt, initial_query, message_optillm_approach = parse_conversation(
        messages
    )
    ...
    base_url = server_config["base_url"]
    # Route: minimal logging; provider selection handled in get_config()

    default_client, api_key = get_config()

    operation, approaches, model = parse_combined_approach(
        model, known_approaches, plugin_approaches
    )
        ...
        request_id = conversation_logger.start_conversation(
            client_request={
                ...
                **{
                    ...
                    if k not in {"messages", "model", "stream", "n"}
                },
            },
            approach=(
                approaches[0]
                if len(approaches) == 1
                else f"{operation}({','.join(approaches)})"
            ),
            model=model,
        )
    ...
    logger.info(
        f"Using approach(es) {approaches}, operation {operation}, with model {model}{request_id_str}"
    )
    if request_id:
    ...
    # Z.ai does not support the 'n' parameter on chat.completions.create.
    # We handle multiple attempts via execute_n_times above, so remove 'n' from provider kwargs.
    try:
        from zai import ZaiClient as _ZaiClient

        if isinstance(client, _ZaiClient):
    ...
    try:
        ...
        if operation == "SINGLE" and approaches[0] == "none":
            ...
            if stream:
                ...
                return Response(
                    generate_streaming_response(extract_contents(result), model),
            ...
            if contains_none:
                raise ValueError(
                    "'none' approach cannot be combined with other approaches"
                )
        ...
        if (
            ...
            if stream:
                if request_id:
                    logger.info(f"Request {request_id}: Completed (streaming response)")
                return Response(
                    generate_streaming_response(extract_contents(response), model),
                    content_type="text/event-stream",
                )
            else:
                if request_id:
    ...
    if isinstance(response, list):
        ...
        if processed_response != response:  # Only process if format changed
            response = [
                msg[-1]["content"] if isinstance(msg, list) and msg else msg
    ...
    if stream:
        return Response(
            generate_streaming_response(response, model),
            content_type="text/event-stream",
        )
    else:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run LLM inference with various approaches."
    )
    ...
    args_env = [
        ...
        (
            ...
            known_approaches + list(plugin_approaches.keys()),
        ),
        (
            "--mcts-simulations",
            "OPTILLM_SIMULATIONS",
        ...
        (
            "--rstar-max-depth",
            "OPTILLM_RSTAR_MAX_DEPTH",
            int,
            3,
            "Maximum depth for rStar algorithm",
        ),
        (
            "--rstar-num-rollouts",
            "OPTILLM_RSTAR_NUM_ROLLOUTS",
            int,
            5,
            "Number of rollouts for rStar algorithm",
        ),
        (
            "--rstar-c",
            "OPTILLM_RSTAR_C",
            float,
            1.4,
            "Exploration constant for rStar algorithm",
        ),
        ...
        (
            "--log-conversations",
            "OPTILLM_LOG_CONVERSATIONS",
            bool,
            False,
            "Enable conversation logging with full metadata",
        ),
        (
            "--conversation-log-dir",
            "OPTILLM_CONVERSATION_LOG_DIR",
            str,
            str(Path.home() / ".optillm" / "conversations"),
    ...
    for arg, env, type_, default, help_text, *extra in args_env:
        ...
        if extra and extra[0]:  # Check if there are choices for this argument
            parser.add_argument(
                arg, type=type_, default=default, help=help_text, choices=extra[0]
            )
        else:
            if type_ == bool:
                # For boolean flags, use store_true action
                parser.add_argument(
                    arg, action="store_true", default=default, help=help_text
                )
            else:
    ...
    ssl_verify_default = os.environ.get("OPTILLM_SSL_VERIFY", "true").lower() in (
        "true",
        "1",
        "yes",
    )
    parser.add_argument(
        "--ssl-verify",
        dest="ssl_verify",
        action="store_true" if ssl_verify_default else "store_false",
...
def main():
    ...
    if server_config.get("batch_mode", False):
        logger.info(
            f"Batch mode enabled: size={server_config['batch_size']}, "
        ...
        def process_batch_requests(batch_requests):
            ...
            for req_data in batch_requests:
                if (
                    req_data["stream"]
                    or req_data["approaches"] != first_req["approaches"]
                    or req_data["operation"] != first_req["operation"]
                    or req_data["model"] != first_req["model"]
                ):
            ...
            for i, req_data in enumerate(batch_requests):
                try:
                    logger.debug(
                        f"Processing batch request {i + 1}/{len(batch_requests)}"
                    )
                    ...
                    if isinstance(result, list):
                        ...
                        ):  # Only process if format changed
                            result = [
                                (
                                    msg[-1]["content"]
    ...
    if server_config["log_conversations"]:
        logger.info(
            f"Conversation logging enabled. Logs will be saved to: {server_config['conversation_log_dir']}"
        )
    ...
    if server_config.get("launch_gui"):
        try:
            ...
            def chat_with_optillm(message, history):
                ...
                try:
                    response = custom_client.chat.completions.create(
                        model=server_config["model"], messages=messages
                    )
            ...
            logger.error(
                "Gradio is required for GUI. Install it with: pip install gradio"
            )
            return
