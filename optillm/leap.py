...
class LEAP:
    ...
    def extract_examples_from_query(self, initial_query: str) -> List[Tuple[str, str]]:
        ...
        if examples_str:
            try:
                ...
                logger.warning(
                    "Parsed JSON does not have the expected structure, using empty list"
                )
    ...
    def generate_mistakes(
        ...
        for question, correct_answer in examples:
            ...
            if generated_answer != correct_answer:
                mistakes.append(
                    (question, generated_reasoning, generated_answer, correct_answer)
                )
        return mistakes

    def generate_low_level_principles(
        ...
        for question, generated_reasoning, generated_answer, correct_answer in mistakes:
            ...
            self.leap_completion_tokens += response.usage.completion_tokens
            self.low_level_principles.append(
                self.extract_output(response.choices[0].message.content)
            )
    ...
    def generate_high_level_principles(self) -> List[str]:
        ...
        self.leap_completion_tokens += response.usage.completion_tokens
        self.high_level_principles = self.extract_output(
    ...
    def solve(self, initial_query: str) -> str:
        ...
        if not examples:
            logger.warning(
                "No examples found in the query. Proceeding with direct answer."
            )
