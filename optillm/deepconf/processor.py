...
class DeepConfProcessor:
    ...
    def __init__(
        ...
        self.confidence_calculator = ConfidenceCalculator(
            window_size=self.config["window_size"], top_k=self.config["top_k"]
        )
        ...
        logger.info(
            f"DeepConf processor initialized with variant: {self.config['variant']}"
        )
    ...
    def generate_single_trace(
        self, messages: List[Dict[str, str]], use_early_termination: bool = False
    ) -> TraceResult:
        """
        ...
        while token_count < self.config["max_tokens_per_trace"]:
            ...
            with torch.no_grad():
                outputs = self.model(
                    input_ids=tokens, past_key_values=kv_cache, use_cache=True
                )
            ...
            if (
                ...
                and token_count >= self.config["min_trace_length"]
                and self.confidence_threshold is not None
            ):

                current_group_confidence = (
                    self.confidence_calculator.get_current_group_confidence()
                )
                if (
                    current_group_confidence is not None
                    and current_group_confidence < self.confidence_threshold
                ):
                    logger.debug(
                        f"Early termination at token {token_count}, "
        ...
        logger.debug(
            f"Generated trace: {token_count} tokens, "
    ...
    def run_warmup_phase(self, messages: List[Dict[str, str]]) -> None:
        ...
        """
        logger.info(
            f"Starting warmup phase with {self.config['warmup_samples']} traces"
        )
        ...
        for i in range(self.config["warmup_samples"]):
            ...
            logger.debug(
                f"Warmup trace {i+1}/{self.config['warmup_samples']} completed"
            )
        ...
        logger.info(
            f"Warmup phase completed. Threshold: {self.confidence_threshold:.4f}"
        )
    ...
    def check_consensus(self, traces: List[TraceResult]) -> Tuple[bool, str, float]:
        ...
        logger.debug(
            f"Consensus check: {consensus_ratio:.3f} "
    ...
    def weighted_majority_vote(
        ...
        for answer, group_traces in answer_groups.items():
            ...
            confidences = [
                trace.confidence_stats["average_confidence"] for trace in group_traces
            ]
        ...
        voting_stats = {
            ...
            "answer_distribution": {
                ans: len(traces) for ans, traces in answer_groups.items()
            },
        }

        logger.info(
            f"Weighted voting completed. Best answer score: {answer_scores[best_answer]:.4f}"
        )
    ...
    def process_online(
        ...
        for trace_num in range(
            self.config["max_traces"] - self.config["warmup_samples"]
        ):
            ...
            # Check consensus
            has_consensus, consensus_answer, consensus_ratio = self.check_consensus(
                all_traces
            )

            logger.debug(
                f"Online trace {trace_num + 1} completed. "
                f"Total traces: {len(all_traces)}, Consensus: {consensus_ratio:.3f}"
            )

            if has_consensus:
                logger.info(
                    f"Consensus reached after {len(all_traces)} traces "
        ...
        processing_stats = {
            ...
            "early_terminations": sum(
                1 for trace in all_traces if trace.terminated_early
            ),
        ...
        logger.info(
            f"DeepConf processing completed. "
