...
class RStar:
    def __init__(
        ...
        logger.debug(
            f"Initialized RStar with model: {model}, max_depth: {max_depth}, num_rollouts: {num_rollouts}"
        )
    ...
    def select_action(self, node: Node) -> Tuple[Node, str]:
        ...
        for child in node.children:
            if child.visits == 0:
                ...
                uct = child.value / child.visits + self.c * math.sqrt(
    ...
    def mutual_consistency(self, trajectory: List[Node]) -> bool:
        ...
        logger.debug(
            f"Mutual consistency check: {'Passed' if is_consistent else 'Failed'}"
        )
    ...
    def select_final_trajectory(self, trajectories: List[List[Node]]) -> List[Node]:
        ...
        if not valid_trajectories:
            logger.warning(
                "No valid trajectories found. Selecting based on value/visits."
            )
        ...
        for answer, conf in valid_answers:
            if answer in answer_counts:
                answer_counts[answer] = (
                    answer_counts[answer][0] + 1,
                    max(answer_counts[answer][1], conf),
                )
            else:
                answer_counts[answer] = (1, conf)

        sorted_answers = sorted(
            answer_counts.items(), key=lambda x: (-x[1][1], -x[1][0])
        )
        best_answer, (count, conf) = sorted_answers[0]

        logger.debug(
            f"Selected best answer: {best_answer} (count: {count}, confidence: {conf})"
        )
        return best_answer
    ...
    def create_prompt(self, state: str, action: str) -> str:
        question = (
            ...
            if hasattr(self, "original_question")
            else "the original question"
        )
        prompts = {
...
This rephrasing should help clarify the problem and guide the solution process.""",
    ...
    def compare_completions(
        ...
        completion_words = set(
            completion.lower().replace(".", "").replace(",", "").split()
        )
        trajectory_words = set(
            remaining_reasoning.lower().replace(".", "").replace(",", "").split()
        )
    ...
    def evaluate(self, node: Node) -> float:
        ...
        try:
            float(answer)
            logger.debug(
                f"Evaluated node. Answer: {answer}, Confidence: {confidence}, Value: {confidence}"
            )
            return confidence  # Return the confidence as the value
        except ValueError:
            logger.debug(
                f"Evaluated node. Answer: {answer}, Confidence: {confidence}, Value: 0.0"
            )
