...
APPROACHES = {
    ...
    "cepo": lambda s, q, c, m: cepo(
        s,
        q,
        c,
        m,
        init_cepo_config(
            {"cepo_config_file": "./optillm/cepo/configs/cepo_config.yaml"}
        ),
    ),
}
...
def run_test_case(test_case: Dict, approaches: List[str], client, model: str) -> Dict:
    ...
    with ThreadPoolExecutor() as executor:
        future_to_approach = {
            executor.submit(
                run_approach, approach, system_prompt, query, client, model
            ): approach
...
def print_summary(results: List[Dict]):
    ...
    for test_result in results:
        ...
        for approach_result in test_result["results"]:
            ...
            print(
                f"  {status} {approach_result['approach']}: {approach_result['time']:.2f}s"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Test different LLM inference approaches."
    )
    parser.add_argument(
        "--test_cases", type=str, default=None, help="Path to test cases JSON file"
    )
    parser.add_argument(
        "--approaches",
        nargs="+",
    ...
    parser.add_argument(
        "--model", type=str, default="gpt-4o-mini", help="Model to use for testing"
    )
    parser.add_argument(
        "--base-url",
        type=str,
        default=None,
        help="The base_url for the OpenAI API compatible endpoint",
    )
    parser.add_argument(
        "--single-test",
        type=str,
        default=None,
        help="Name of a single test case to run",
    )
    ...
    results = run_tests(
        test_cases, args.approaches, client, args.model, args.single_test
    )
